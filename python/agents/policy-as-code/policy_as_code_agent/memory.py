...
from .config import CORE_POLICIES_DOC_REF, EMBEDDING_MODEL_NAME, ENABLE_MEMORY_BANK, FIRESTORE_COLLECTION_EXECUTIONS, FIRESTORE_COLLECTION_POLICIES, FIRESTORE_DATABASE, LOCATION, PROJECT_ID
db = None
if ENABLE_MEMORY_BANK:
    try:
        db = firestore.Client(project=PROJECT_ID, database=FIRESTORE_DATABASE)
        logging.info(f'Firestore initialized and verified for project: {db.project}, database: {FIRESTORE_DATABASE}')
    except Exception as e:
        logging.warning(f'Firestore initialization failed or database not found. Memory bank will be disabled. Error: {e}')
        db = None
else:
...
def get_active_core_policies() -> dict:
    ...
    if not db:
        return {'status': 'success', 'source': 'default', 'message': 'Memory bank disabled. Using default core policies.', 'policies': []}
    try:
        doc = db.document(CORE_POLICIES_DOC_REF).get()
        if doc.exists:
            return {'status': 'success', 'source': 'firestore', 'policies': doc.to_dict().get('policies', [])}
        else:
            return {'status': 'success', 'source': 'default', 'message': 'No core policies saved in memory. Using defaults.', 'policies': []}
    except Exception as e:
...
def save_core_policies(policies: list[str]) -> dict:
    ...
    try:
        db.document(CORE_POLICIES_DOC_REF).set({'policies': policies, 'updated_at': datetime.datetime.now()})
        return {'status': 'success', 'message': 'Core policies saved successfully.', 'policies': policies}
    except Exception as e:
...
def add_core_policy(policy: str) -> dict:
    ...
    try:
        ...
        doc_ref.set({'policies': firestore.ArrayUnion([policy]), 'updated_at': datetime.datetime.now()}, merge=True)
        updated_doc = doc_ref.get()
        policies = updated_doc.to_dict().get('policies', [])
        return {'status': 'success', 'message': f"Added policy: '{policy}'", 'policies': policies}
    except Exception as e:
...
def remove_core_policy(policy: str) -> dict:
    ...
    try:
        doc_ref = db.document(CORE_POLICIES_DOC_REF)
        doc_ref.set({'policies': firestore.ArrayRemove([policy]), 'updated_at': datetime.datetime.now()}, merge=True)
        updated_doc = doc_ref.get()
        policies = updated_doc.to_dict().get('policies', [])
        return {'status': 'success', 'message': f"Removed policy: '{policy}'", 'policies': policies}
    except Exception as e:
        return {'status': 'error', 'message': f'Failed to remove core policy: {e}'}

@retry(wait=wait_exponential(multiplier=1, min=4, max=10), stop=stop_after_attempt(3))
def find_policy_in_memory(query: str, source: str, version: str='latest', author: str | None=None, start_date: str | None=None, end_date: str | None=None) -> dict:
    """
    ...
    """
    if not db:
        return {'status': 'error', 'message': 'Memory bank is disabled. To enable long-term procedural memory, set ENABLE_MEMORY_BANK=True in .env and configure Firestore. See docs/MEMORY_INTEGRATION.md for details.'}
    query_embedding = _get_embedding(query)
    if not query_embedding:
        return {'status': 'error', 'message': 'Failed to generate embedding for query.'}
    policies_ref = db.collection(COLLECTION_NAME)
    try:
        vector_query = policies_ref.find_nearest(vector_field='embedding', query_vector=Vector(query_embedding), distance_measure=DistanceMeasure.COSINE, limit=10, distance_result_field='similarity_distance')
        results = list(vector_query.stream())
    except FailedPrecondition as e:
        if 'index' in str(e).lower():
            return {'status': 'error', 'message': f'Firestore Vector Index missing. Please create the index using the link in the logs. Error: {e}'}
    ...
    if start_date and end_date:
        try:
            ...
            filtered_matches = [m for m in filtered_matches if start <= datetime.datetime.fromisoformat(m['created_at']) <= end]
        except Exception:
            pass
    if not filtered_matches:
        return {'status': 'not_found', 'message': 'No policies found matching criteria after vector search.'}
    filtered_matches.sort(key=lambda x: x['similarity'], reverse=True)
    best_match = filtered_matches[0]
    try:
        doc_ref = db.collection(COLLECTION_NAME).where('policy_id', '==', best_match['policy_id']).where('version', '==', best_match['version']).limit(1).get()
        if doc_ref:
            doc_ref[0].reference.update({'last_used': datetime.datetime.now()})
    except Exception:
        pass
    return {'status': 'found', 'similarity': best_match['similarity'], 'policy': best_match}

@retry(wait=wait_exponential(multiplier=1, min=4, max=10), stop=stop_after_attempt(3))
def save_policy_to_memory(natural_language_query: str, policy_code: str, source: str, author: str='user', policy_id: str | None=None) -> dict:
    """
    ...
    if not embedding_list:
        return {'status': 'error', 'message': 'Could not generate embedding for policy.'}
    new_version = 1
    if policy_id:
        existing_versions = list(db.collection(COLLECTION_NAME).where('policy_id', '==', policy_id).stream())
    ...
    doc_data = {'policy_id': policy_id, 'version': new_version, 'query': natural_language_query, 'embedding': Vector(embedding_list), 'code': policy_code, 'source': source, 'author': author, 'last_used': now, 'created_at': now, 'ratings': [], 'feedback': []}
    try:
        db.collection(COLLECTION_NAME).add(doc_data)
        return {'status': 'success', 'message': f'Policy saved as version {new_version} with ID {policy_id}.', 'policy_id': policy_id, 'version': new_version}
    except Exception as e:
...
def list_policy_versions(policy_id: str) -> dict:
    ...
    if not db:
        return {'status': 'error', 'message': 'Memory bank is disabled. To enable long-term procedural memory, set ENABLE_MEMORY_BANK=True in .env and configure Firestore. See docs/MEMORY_INTEGRATION.md for details.'}
    try:
        docs = db.collection(COLLECTION_NAME).where('policy_id', '==', policy_id).stream()
        versions = []
        for doc in docs:
            versions.append(_policy_to_dict(doc))
        if not versions:
            return {'status': 'not_found', 'message': f"No policy found with ID '{policy_id}'."}
...
def get_policy_by_id(policy_id: str, version: int) -> dict:
    ...
    if not db:
        return {'status': 'error', 'message': 'Memory bank is disabled. To enable long-term procedural memory, set ENABLE_MEMORY_BANK=True in .env and configure Firestore. See docs/MEMORY_INTEGRATION.md for details.'}
    try:
        docs = list(db.collection(COLLECTION_NAME).where('policy_id', '==', policy_id).where('version', '==', int(version)).limit(1).stream())
        if not docs:
            return {'status': 'not_found', 'message': f"Policy with ID '{policy_id}' and version '{version}' not found."}
...
def prune_memory(days_to_keep: int=30) -> dict:
    ...
    try:
        docs = db.collection(COLLECTION_NAME).where('last_used', '<', cutoff_date).stream()
        deleted_count = 0
        batch = db.batch()
        for doc in docs:
        ...
        if deleted_count % 400 != 0:
            batch.commit()
        return {'status': 'success', 'message': f'Pruned {deleted_count} old policies.'}
    except Exception as e:
        return {'status': 'error', 'message': f'Error pruning memory: {e}'}

def rate_policy(policy_id: str, version: int, rating: int, feedback: str | None=None) -> dict:
    """Rates a policy and adds feedback in Firestore."""
    max_rating = 5
    if not 1 <= rating <= max_rating:
        return {'status': 'error', 'message': f'Rating must be between 1 and {max_rating}.'}
    if not db:
        return {'status': 'error', 'message': 'Memory bank is disabled. To enable long-term procedural memory, set ENABLE_MEMORY_BANK=True in .env and configure Firestore. See docs/MEMORY_INTEGRATION.md for details.'}
    try:
        docs = list(db.collection(COLLECTION_NAME).where('policy_id', '==', policy_id).where('version', '==', int(version)).limit(1).stream())
...
@retry(wait=wait_exponential(multiplier=1, min=4, max=10), stop=stop_after_attempt(3))
def log_policy_execution(policy_id: str, version: int, status: str, source: str, violations: list[dict[str, Any]] | None=None, summary: str='') -> dict:
    """
    ...
    """
    if not db:
        return {'status': 'success', 'message': 'Memory bank disabled. Execution not logged.'}
    now = datetime.datetime.now()
    violation_count = len(violations) if violations else 0
    violated_resources = set()
    if violations:
        for v in violations:
            res_name = v.get('resource_name') or v.get('table_name') or v.get('asset_name') or v.get('dataset_name')
            if res_name:
                violated_resources.add(str(res_name))
    execution_data = {'policy_id': policy_id, 'version': version, 'timestamp': now, 'status': status, 'violation_count': violation_count, 'source': source, 'summary': summary, 'violated_resources': list(violated_resources)}
    try:
        ...
        if docs:
            ...
            if status == 'violations_found':
                updates['total_violations_detected'] = firestore.Increment(violation_count)
            if status == 'failure':
...
@retry(wait=wait_exponential(multiplier=1, min=4, max=10), stop=stop_after_attempt(3))
def analyze_execution_history(query_type: str='summary', days: int=30, resource_name: str | None=None) -> dict:
    """
    ...
    try:
        if query_type == 'resource_search':
            if not resource_name:
                return {'status': 'error', 'message': 'resource_name is required for resource_search.'}
            query = db.collection(FIRESTORE_COLLECTION_EXECUTIONS).where('timestamp', '>=', cutoff_date)
            ...
            for doc in results:
                ...
                if not violated_resources:
                    continue
                if any((resource_name_lower in str(r).lower() for r in violated_resources)):
            ...
            for m in matches:
                ...
                if cache_key not in policy_cache:
                    try:
                        docs = list(db.collection(COLLECTION_NAME).where('policy_id', '==', pid).where('version', '==', int(ver)).limit(1).stream())
                        if docs:
            ...
            query = db.collection(COLLECTION_NAME).order_by('total_violations_detected', direction=firestore.Query.DESCENDING).limit(10)
            results = query.stream()
            top_policies = []
            for doc in results:
                p = _policy_to_dict(doc)
                if p.get('total_violations_detected', 0) > 0:
                    top_policies.append({'policy_id': p.get('policy_id'), 'query': p.get('query'), 'total_violations': p.get('total_violations_detected'), 'last_run': p.get('last_run')})
            ...
            top_resources = [{'resource_name': r[0], 'total_violations': r[1]} for r in sorted_resources[:10]]
            ...
            for doc in results:
                ...
                daily_stats[day_str]['violations_detected'] += data.get('violation_count', 0)
            return {'status': 'success', 'data': daily_stats}
    except Exception as e:
        return {'status': 'error', 'message': f'Analysis failed: {e}'}

@retry(wait=wait_exponential(multiplier=1, min=4, max=10), stop=stop_after_attempt(3))
def get_execution_history(days: int=7, status: str | None=None, policy_id: str | None=None) -> dict:
    """
    ...
    try:
        ...
        for doc in results:
            ...
            history.append(data)
        if not history:
            return {'status': 'success', 'history': [], 'message': 'No execution history found for this criteria.'}
