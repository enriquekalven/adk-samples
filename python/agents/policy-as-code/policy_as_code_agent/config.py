...
FIRESTORE_COLLECTION_EXECUTIONS = os.getenv('FIRESTORE_COLLECTION_EXECUTIONS', 'policy_executions')
CORE_POLICIES_DOC_REF = os.getenv('CORE_POLICIES_DOC_REF', 'configurations/core_policies')
...
DEFAULT_CORE_POLICIES = ['All tables in analytics_dataset & finance_dataset must be partitioned', 'No tables should have a column of byte or boolean datatype', "The 'data_owner' label must be present on all tables.", 'Table names must be in snake_case.', "For every table in the marketing_dataset that has 'performance' in its name, there must exist a table in the sales_dataset with 'summary' in its name, AND all tables in the marketing_dataset must use snake_case for all column names."]