...
def load_metadata(gcs_uri: str):
    ...
    try:
        ...
        metadata = [json.loads(line) for line in content.decode('utf-8').splitlines() if line.strip()]
        return metadata
...
def get_content_from_gcs_for_schema(gcs_uri: str) -> dict:
    ...
    try:
        ...
        if is_directory:
            blobs = list(storage_client.list_blobs(bucket, prefix=blob_prefix, max_results=10))
            files_in_dir = [f'gs://{bucket_name}/{b.name}' for b in blobs if not b.name.endswith('/')]
            if not files_in_dir:
                return {'status': 'error', 'error_message': f'No files found in GCS directory {gcs_uri} to generate schema.'}
            schema_gcs_uri = files_in_dir[0]
            logging.info(f'Directory detected. Using first file for schema: {schema_gcs_uri}')
    except Exception as e:
    ...
    try:
        ...
        return {'status': 'error', 'error_message': f'Error downloading file {schema_gcs_uri} for schema generation: {e}'}