...
def generate_sample_values_str(metadata_sample: list) -> str:
    ...
    if not metadata_sample:
        return '{}'
    try:
        most_representative_entry = max(metadata_sample, key=lambda item: len(json.dumps(item)))
...
def get_json_schema_from_content(content: str):
    ...
    all_schemas = [generate_schema_from_obj(item) for item in data if isinstance(item, dict)]
    if not all_schemas:
        return {}
