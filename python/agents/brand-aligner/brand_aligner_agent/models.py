...
class Criterion(BaseModel):
    criterion_id: str = Field(default_factory=lambda: f'criterion_{uuid.uuid4().hex[:10]}', description='Unique identifier for the criterion')
    name: str
    criterion_value: str
    severity: Severity
    category: str
...
class GuidelineBase(BaseModel):
    guideline_id: str = Field(default_factory=lambda: f'guide_{uuid.uuid4().hex[:10]}', description='Unique identifier for the guideline')
    name: str
    description: str
