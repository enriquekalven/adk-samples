...
from vertexai.preview.evaluation import CustomOutputConfig, EvalTask, PointwiseMetric, RubricBasedMetric, RubricGenerationConfig
from .models import Asset, AssetEvaluation, Category, Criterion, CriterionRubric, CriterionVerdict, Guideline, GuidelineResponse, GuidelineVerdict, Severity
...
class GuidelineService:
    ...
    async def extract_guideline_from_doc(self, file_uri: str, mime_type: str) -> GuidelineResponse:
        ...
        try:
            ...
            prompt = GUIDELINE_EXTRACTION_PROMPT.format(categories=', '.join(category_enums))
            guideline_file = genai_types.Part.from_uri(file_uri=file_uri, mime_type=mime_type)
            contents = genai_types.Content(role='user', parts=[genai_types.Part.from_text(text=prompt), guideline_file])
            logger.info('Sending document to Gemini for extraction: %s...', file_uri)
            response = await client.aio.models.generate_content(model=self.model_name, contents=contents, config={'response_mime_type': 'application/json', 'response_schema': GuidelineResponse.model_json_schema()})
...
_QUESTION_REGEX = re.compile('Question:(.*?)Verdict:', re.DOTALL | re.IGNORECASE)
_VERDICT_REGEX = re.compile('Verdict:\\s*(Yes|No|N/A)', re.IGNORECASE)
_JUSTIFICATION_REGEX = re.compile('Justification:(.*)', re.DOTALL | re.IGNORECASE)

def _parse_json_to_rubrics(json_response: str):
    """Custom parsing function for the rubric generation step (Yes/No)."""
    clean_response = re.sub('(.*```json|```.*)', '', json_response.strip(), flags=re.DOTALL)
    try:
        data = json.loads(clean_response)
        rubrics = []
        rubrics_for_validator = []
        for qa in data.get('qas', []):
            record = CriterionRubric(criterion_id=qa.get('criterion_id', 'unknown'), question=qa['question'], gt_answer=qa.get('answer', 'yes').lower(), question_justification=qa.get('justification', ''), guideline_id='')
            rubrics.append(record)
            rubrics_for_validator.append(f'<question>{record.question}</question>')
        return {'questions': '\n'.join(rubrics_for_validator), 'rubrics_internal': rubrics}
    except (json.JSONDecodeError, KeyError) as e:
        logger.exception(f'Error parsing rubric generation JSON: {clean_response}')
    ...
    for result in results:
        ...
        for block in question_blocks:
            ...
            if question_match and verdict_match and justification_match:
                ...
                verdicts[question] = {'verdict': verdict, 'justification': justification}
...
def _compute_scores_from_result(df: pd.DataFrame, guideline: Guideline | None):
    ...
    scores = []
    criterion_verdicts = []
    severity_map = {c.criterion_id: c.severity for c in guideline.criteria} if guideline else {}
    criterion_name_map = {c.criterion_id: c.name for c in guideline.criteria} if guideline else {}
    criterion_category_map = {c.criterion_id: c.category for c in guideline.criteria} if guideline else {}
    for _, row in df.iterrows():
        ...
        for rubric in rubrics:
            ...
            justification = verdict_dict.get('justification', rubric.question_justification)
            ...
            criterion_verdicts.append(CriterionVerdict(guideline_id=rubric.guideline_id, criterion_id=rubric.criterion_id, criterion_name=c_name, category=c_category, question=rubric.question, gt_answer=gt_answer, question_justification=rubric.question_justification, verdict=verdict, justification=justification))
            if verdict == 'n/a':
                continue
...
class EvalService:
    ...
    def __init__(self, project_id: str, location: str, model_name: str, bucket_name: str, batch_size: int=50):
        vertexai.init(project=project_id, location=location)
        self.model_name = model_name
        self.gecko_metric = RubricBasedMetric(generation_config=None, critique_metric=PointwiseMetric(metric='gecko_metric', metric_prompt_template=RUBRIC_VALIDATOR_PROMPT, custom_output_config=CustomOutputConfig(return_raw_output=True, parsing_fn=_parse_validator_results)))
        self.batch_size = batch_size
        self.filtering_model = GenerativeModel(self.model_name)
        self.storage_client = storage.Client(project=project_id)
        self.bucket = self.storage_client.bucket(bucket_name)

    def _get_asset_type(self, asset_uri: str) -> Literal['video', 'image', 'unknown']:
    ...
    def _filter_relevant_criteria(self, all_criteria: list[Criterion], asset_type: str) -> list[Criterion]:
        ...
        if not all_criteria or asset_type == 'unknown':
            return all_criteria
        criteria_dict_list = [{'criterion_id': c.criterion_id, 'criterion_value': c.criterion_value, 'severity': c.severity.name} for c in all_criteria]
        criteria_json = json.dumps(criteria_dict_list, indent=2)
        prompt = CRITERIA_FILTERING_PROMPT.format(asset_type=asset_type, criteria_json=criteria_json)
        try:
            response = self.filtering_model.generate_content(prompt)
            clean_response = re.sub('(.*```json|```.*)', '', response.text.strip(), flags=re.DOTALL)
            result_json = json.loads(clean_response)
            relevant_ids = set(result_json.get('relevant_criterion_ids', []))
            if not relevant_ids:
                logger.warning('Criteria filtering returned no relevant IDs. Proceeding with all criteria.')
                return all_criteria
            filtered_criteria = [c for c in all_criteria if c.criterion_id in relevant_ids]
            logger.info(f"Filtered criteria for asset type '{asset_type}'. Original count: {len(all_criteria)}, New count: {len(filtered_criteria)}")
            return filtered_criteria
        except (json.JSONDecodeError, exceptions.GoogleAPICallError) as e:
            logger.exception(f'Error during criteria filtering LLM call: {e}. Proceeding with all criteria.')
            return all_criteria

    def evaluate_asset(self, asset: Asset, guidelines: list[Guideline], additional_guidance: str, user_id: str, app_name: str) -> AssetEvaluation:
        ...
        guideline_id = 'dsg_prompt_adherence_and_gqm_quality' if valid_prompt else 'gqm_quality'
        dsg_score, dsg_verdicts = self._run_evaluation_task(prompt_template=DSG_RUBRIC_GENERATION_PROMPT, prompt_params={'source_prompt': prompt}, asset_uri=asset_uri, reference_image_paths=video_reference_image_uris, guideline_id=guideline_id, user_id=user_id, app_name=app_name, use_cache=False)
        guideline_verdicts.append(GuidelineVerdict(guideline_id=guideline_id, mean_score=dsg_score, verdicts=dsg_verdicts))
        if dsg_score != -1:
            guideline_scores.append(dsg_score)
        for guideline in guidelines:
            if not guideline.criteria:
                logger.warning(f'Guideline {guideline.guideline_id} has no criteria, skipping.')
                continue
            relevant_criteria = self._filter_relevant_criteria(guideline.criteria, asset_type) if len(guideline.criteria) > self.batch_size else guideline.criteria
            filtered_guideline = guideline.model_copy(update={'criteria': relevant_criteria})
            criteria_str = '\n'.join([f'- {c.criterion_id} (Severity: {c.severity.name}): {c.criterion_value}' for c in filtered_guideline.criteria])
            bas_score, bas_verdicts = self._run_evaluation_task(prompt_template=BAS_RUBRIC_GENERATION_PROMPT, prompt_params={'criteria': criteria_str, 'additional_guidance': additional_guidance or 'None'}, asset_uri=asset_uri, reference_image_paths=video_reference_image_uris, guideline_id=filtered_guideline.guideline_id, guideline=filtered_guideline, user_id=user_id, app_name=app_name, use_cache=True)
            guideline_verdicts.append(GuidelineVerdict(guideline_id=filtered_guideline.guideline_id, mean_score=bas_score, verdicts=bas_verdicts))
            if bas_score != -1:
                guideline_scores.append(bas_score)
        asset_final_score = np.mean(guideline_scores) if guideline_scores else 0
        return AssetEvaluation(asset_id=asset_id, asset_name=asset.asset_name, description=prompt, guideline_verdicts=guideline_verdicts, final_score=asset_final_score)

    def _run_batch_evaluation(self, batch_dataset: pd.DataFrame) -> pd.DataFrame | None:
        ...
        batch_size = len(batch_dataset.at[0, 'rubrics_internal']) if 'rubrics_internal' in batch_dataset.columns else 'unknown'
        for attempt in range(max_retries):
            try:
                eval_result = eval_task.evaluate(response_column_name='generated_asset', retry_timeout=300)
                logger.info(f'Successfully evaluated batch with {batch_size} rubrics on attempt {attempt + 1}.')
                return eval_result.metrics_table
            except (exceptions.ResourceExhausted, exceptions.DeadlineExceeded) as e:
                if attempt < max_retries - 1:
                    delay = initial_delay * backoff_factor ** attempt
                    logger.warning(f'Retryable error on batch, attempt {attempt + 1}/{max_retries}. Retrying in {delay} seconds. Error: {e}')
                    time.sleep(delay)
                else:
                    logger.error(f'Failed to evaluate batch after {max_retries} attempts. Error: {e}')
                    return None
            except Exception as e:
                logger.error(f'An unexpected error occurred on batch attempt {attempt + 1}: {e}')
                return None
        return None

    def _get_or_generate_rubrics(self, prompt_template: str, prompt_params: dict, asset_uri: str, reference_image_paths: list[str], guideline_id: str, user_id: str, app_name: str, use_cache: bool=True) -> list[CriterionRubric]:
        ...
        if use_cache:
            ...
            if blob.exists():
                logger.info(f'Cache hit: Loading rubrics for {guideline_id} from {blob_name}')
                try:
                    json_data = blob.download_as_text()
                    data = json.loads(json_data)
                    return [CriterionRubric(**item) for item in data]
                except Exception as e:
                    logger.warning(f'Failed to load cached rubrics: {e}. Regenerating.')
        logger.info(f'Generating rubrics for {guideline_id} (cache used: {use_cache})')
        rubric_gen_config = RubricGenerationConfig(prompt_template=prompt_template.format(**prompt_params), parsing_fn=_parse_json_to_rubrics, model=GenerativeModel(self.model_name))
        self.gecko_metric.generation_config = rubric_gen_config
        generated_asset_json_str = _create_multimodal_json_string([asset_uri])
        source_images_json_str = _create_multimodal_json_string(reference_image_paths)
        eval_dataset = pd.DataFrame([{'generated_asset': generated_asset_json_str, 'source_images': source_images_json_str}])
        eval_dataset_with_rubrics = self.gecko_metric.generate_rubrics(eval_dataset)
        if eval_dataset_with_rubrics.empty or 'rubrics_internal' not in eval_dataset_with_rubrics.columns:
        ...
        if use_cache and blob:
            try:
                ...
                blob.upload_from_string(json.dumps(serialized_rubrics, indent=2), content_type='application/json')
                logger.info(f'Saved generated rubrics to cache: {blob.name}')
            except Exception as e:
                logger.error(f'Failed to save rubrics to cache: {e}')
        return rubrics

    def _execute_evaluation(self, rubrics: list[CriterionRubric], asset_uri: str, reference_image_paths: list[str], guideline: Guideline | None):
        """Executes the evaluation using the provided rubrics."""
        generated_asset_json_str = _create_multimodal_json_string([asset_uri])
        source_images_json_str = _create_multimodal_json_string(reference_image_paths)
        eval_dataset = pd.DataFrame([{'generated_asset': generated_asset_json_str, 'source_images': source_images_json_str}])
        eval_dataset['rubrics_internal'] = eval_dataset['rubrics_internal'].astype(object)
        eval_dataset['rubrics'] = ''
        rubric_batches = [rubrics[i:i + self.batch_size] for i in range(0, len(rubrics), self.batch_size)]
        all_metrics_tables = []
        with concurrent.futures.ThreadPoolExecutor() as executor:
            future_to_batch = {}
            for batch in rubric_batches:
                rubrics_str = '\n'.join([f'<question>{r.question}</question>' for r in batch])
                batch_dataset = eval_dataset.copy()
                batch_dataset.at[0, 'rubrics_internal'] = batch
                batch_dataset.at[0, 'rubrics'] = rubrics_str
                future = executor.submit(self._run_batch_evaluation, batch_dataset)
        ...
        if not metrics_table.empty and 'rubrics_internal' in metrics_table.columns:
            for row_index in metrics_table.index:
                new_rubrics = [CriterionRubric(guideline_id=row.guideline_id, criterion_id=row.criterion_id, question=row.question, gt_answer=row.gt_answer, question_justification=row.question_justification) for row in metrics_table.loc[row_index, 'rubrics_internal']]
                metrics_table.at[row_index, 'rubrics_internal'] = new_rubrics
        return _compute_scores_from_result(metrics_table, guideline)

    def _run_evaluation_task(self, prompt_template: str, prompt_params: dict, asset_uri: str, reference_image_paths: list[str], guideline_id: str, user_id: str, app_name: str, guideline: Guideline | None=None, use_cache: bool=True):
        """Helper to run evaluation task with caching support."""
        try:
            rubrics = self._get_or_generate_rubrics(prompt_template=prompt_template, prompt_params=prompt_params, asset_uri=asset_uri, reference_image_paths=reference_image_paths, guideline_id=guideline_id, user_id=user_id, app_name=app_name, use_cache=use_cache)
            if not rubrics:
                return (0.0, [])
            logger.info('%d rubrics ready for guideline %s.', len(rubrics), guideline_id)
            return self._execute_evaluation(rubrics, asset_uri, reference_image_paths, guideline)
        except Exception:
            logger.exception(f"Error during EvalTask for guideline '{guideline_id}'")
            return (0.0, [])
...
def _get_mime_type(file_path: str) -> str:
    ...
    file_name = f if '.' in f else str(p.parents[0]).replace(f'{p.parents[1]!s}/', '')
    ...
    if not mime_type:
        ...
        if any((file_name.lower().endswith(ext) for ext in ['.jpg', '.jpeg'])):
            return 'image/jpeg'
        if any((file_name.lower().endswith(ext) for ext in ['.mp4', '.mov', '.avi'])):
            return 'video/mp4'
...
def _create_multimodal_json_string(file_paths: list[str]) -> str:
    ...
    for path in file_paths:
        parts_list.append({'file_data': {'file_uri': path, 'mime_type': _get_mime_type(path)}})
