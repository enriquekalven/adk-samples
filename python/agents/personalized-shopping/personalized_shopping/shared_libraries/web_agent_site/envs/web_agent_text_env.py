...
from ..engine.engine import ACTION_TO_TEMPLATE, BACK_TO_SEARCH, END_BUTTON, NEXT_PAGE, PREV_PAGE, get_product_per_page, get_top_n_product_from_keywords, init_search_engine, load_products, map_action_to_html, parse_action
from ..engine.goal import get_goals, get_reward
from ..utils import DEFAULT_FILE_PATH, FEAT_CONV, FEAT_IDS, random_idx
app = Flask(__name__)
...
class WebAgentTextEnv(gym.Env):
    ...
    def get_available_actions(self):
        ...
        return {'has_search_bar': has_search_bar, 'clickables': list(self.text_to_clickable.keys())}
    ...
    def state(self):
        ...
        state.
        """
        return {'url': self.browser.current_url, 'html': self.browser.page_source, 'instruction_text': self.instruction_text}
...
class SimServer:
    ...
    def search_results(self, session_id, **kwargs):
        ...
        old_time = time.time()
        html = map_action_to_html('search', session_id=session_id, products=products, keywords=session['keywords'], page=page, total=len(top_n_products), instruction_text=self.assigned_instruction_text)
    ...
    def item_page(self, session_id, **kwargs):
        ...
        html = map_action_to_html('click', session_id=session_id, product_info=product_info, keywords=session['keywords'], page=session['page'], asin=session['asin'], options=session['options'], instruction_text=self.assigned_instruction_text, show_attrs=self.show_attrs)
        return (html, url)
    ...
    def item_sub_page(self, session_id, **kwargs):
        ...
        html = map_action_to_html(f'click[{clickable_name}]', session_id=session_id, product_info=product_info, keywords=session['keywords'], page=session['page'], asin=session['asin'], options=session['options'], instruction_text=self.assigned_instruction_text)
        return (html, url)
    ...
    def done(self, session_id, **kwargs):
        ...
        html = map_action_to_html(f'click[{END_BUTTON}]', session_id=session_id, reward=reward, asin=session['asin'], options=session['options'], instruction_text=self.assigned_instruction_text)
    ...
    def receive(self, session_id, current_url, session_int=None, **kwargs):
        ...
        with app.app_context(), app.test_request_context():
            ...
            if not kwargs:
                ...
                self.user_sessions[session_id].update({'keywords': None, 'page': None, 'asin': None, 'asins': set(), 'options': {}, 'actions': defaultdict(int)})
...
class SimBrowser:
    ...
    def search(self, keywords):
        ...
        return status
register(id='WebAgentTextEnv-v0', entry_point='personalized_shopping.shared_libraries.web_agent_site.envs.web_agent_text_env:WebAgentTextEnv')