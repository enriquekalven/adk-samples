...
def get_synthetic_goals(all_products, product_prices):
    ...
    for product in all_products:
        ...
        for combination in combinations:
            ...
            product_goals.append({'asin': asin, 'category': product['category'], 'query': product['query'], 'name': product['name'], 'product_category': product['product_category'], 'instruction_text': f'{instruction_text}{option_text}{price_text}', 'attributes': attributes, 'price_upper': price_upper, 'goal_options': goal_options, 'title': product['Title']})
            for att in attributes:
                cnt_atts[att] += 1
        goals += product_goals
    for goal in goals:
...
def get_type_reward(purchased_product, goal):
    ...
    category_match = len(set(purchased_product_category) & set(goal_product_category)) >= min_category_matches
    ...
    if title_score == 0.0:
        r_type = 0.0
    return {'r_type': r_type, 'query_match': query_match, 'category_match': category_match, 'title_score': title_score}
