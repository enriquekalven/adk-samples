...
from a2a.types import GetTaskRequest, GetTaskResponse, MessageSendParams, SendMessageRequest, SendMessageResponse, SendMessageSuccessResponse, Task, TaskQueryParams, TaskState
...
def create_send_message_payload(text: str, task_id: str | None=None, context_id: str | None=None) -> dict[str, Any]:
    ...
    payload: dict[str, Any] = {'message': {'role': 'user', 'parts': [{'kind': 'text', 'text': text}], 'messageId': uuid4().hex}}
    if task_id:
...
async def run_single_turn_test(client: A2AClient) -> None:
    ...
    get_response: GetTaskResponse = await client.get_task(get_request, timeout=10, timeout=10)
...
async def run_multi_turn_test(client: A2AClient) -> None:
    ...
    request1 = SendMessageRequest(id=str(uuid4()), params=MessageSendParams(**first_turn_payload))
    ...
    if isinstance(first_turn_response.root, SendMessageSuccessResponse) and isinstance(first_turn_response.root.result, Task):
        ...
        if task.status.state == TaskState.input_required and context_id:
            ...
            second_turn_payload = create_send_message_payload('in GBP', task.id, context_id)
            request2 = SendMessageRequest(id=str(uuid4()), params=MessageSendParams(**second_turn_payload))
...
async def main() -> None:
    ...
    try:
        async with httpx.AsyncClient() as httpx_client:
            ...
            agent_card = await resolver.get_agent_card(timeout=10, timeout=10)
