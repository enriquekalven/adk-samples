...
class Config(BaseSettings):
    ...
    model_config = SettingsConfigDict(env_file=os.path.join(os.path.dirname(os.path.abspath(__file__)), '../.env'), env_prefix='GOOGLE_', case_sensitive=True, extra='ignore')
    agent_settings: AgentModel = Field(default=AgentModel())
    app_name: str = 'youtube_analyst_app'
    CLOUD_PROJECT: str = Field(default='', description='Google Cloud Project ID')
    CLOUD_LOCATION: str = Field(default='global')
    GENAI_USE_VERTEXAI: str = Field(default='1')
    API_KEY: str = Field(default='', description='Google API Key for YouTube Data API')
