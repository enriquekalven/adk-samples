...
def search_youtube(query: str, max_results: int=10, published_after: str=''):
    """
    ...
    try:
        ...
        kwargs = {'q': query, 'type': 'video', 'part': 'id,snippet', 'maxResults': max_results}
        if published_after:
        ...
        for search_result in search_response.get('items', []):
            if search_result['id']['kind'] == 'youtube#video':
                results.append({'title': search_result['snippet']['title'], 'videoId': search_result['id']['videoId'], 'channelTitle': search_result['snippet']['channelTitle'], 'description': search_result['snippet']['description']})
        return results
...
def get_video_details(video_ids: list):
    ...
    try:
        ...
        video_response = youtube.videos().list(id=ids_string, part='snippet,statistics,contentDetails').execute()
        results = []
        for video_result in video_response.get('items', []):
            stats = video_result['statistics']
            snippet = video_result['snippet']
            results.append({'videoId': video_result['id'], 'title': snippet['title'], 'channelTitle': snippet['channelTitle'], 'viewCount': stats.get('viewCount', 0), 'likeCount': stats.get('likeCount', 0), 'commentCount': stats.get('commentCount', 0), 'publishedAt': snippet['publishedAt'], 'tags': snippet.get('tags', [])})
        return results
    except HttpError as e:
        logger.error(f'An HTTP error {e.resp.status} occurred:\n{e.content}')
        return []
...
def get_channel_details(channel_ids: list):
    ...
    try:
        ...
        channel_response = youtube.channels().list(id=ids_string, part='snippet,statistics').execute()
        results = []
        for channel_result in channel_response.get('items', []):
            stats = channel_result['statistics']
            snippet = channel_result['snippet']
            results.append({'channelId': channel_result['id'], 'title': snippet['title'], 'description': snippet['description'], 'subscriberCount': stats.get('subscriberCount', 0), 'videoCount': stats.get('videoCount', 0), 'viewCount': stats.get('viewCount', 0)})
        return results
    except HttpError as e:
        logger.error(f'An HTTP error {e.resp.status} occurred:\n{e.content}')
        return []
...
def get_video_comments(video_id: str, max_results: int=20):
    ...
    try:
        youtube = get_youtube_client()
        comment_response = youtube.commentThreads().list(videoId=video_id, part='snippet', maxResults=max_results, textFormat='plainText').execute()
        comments = []
...
def calculate_engagement_metrics(view_count: int, like_count: int, comment_count: int, subscriber_count: int):
    """
    ...
    return {'engagement_rate': round(engagement_rate, 2), 'active_rate': round(active_rate, 2)}

def calculate_match_score(subscribers: int, engagement_rate: float, active_rate: float, sentiment_score: float=0.0):
    """
    ...
    total_score = sub_score * 0.4 + eng_score * 0.3 + active_score * 0.2 + sent_score * 0.1
...
def analyze_sentiment_heuristic(text: str):
    ...
    if not text:
        return 0
    positives = ['great', 'good', 'love', 'amazing', 'best', 'nice', 'helpful', 'excellent', 'wow']
    negatives = ['bad', 'hate', 'worst', 'terrible', 'awful', 'boring', 'useless', 'poor']
...
async def render_html(html_content: str, filename: str, tool_context: ToolContext):
    ...
    try:
        ...
        try:
            await tool_context.save_artifact(filename.replace('.', '_'), types.Part.from_bytes(data=html_content.encode('utf-8'), mime_type='text/html'))
