...
default_jailbreak_safety_agent = LlmAgent(model='gemini-2.5-flash-lite', name='jailbreak_safety_agent', instruction=prompts.JAILBREAK_FILTER_INSTRUCTION, context_cache_config=ContextCacheConfig(min_tokens=2048, ttl_seconds=600))
...
class LlmAsAJudge(BasePlugin):
    ...
    def __init__(self, judge_agent: LlmAgent=default_jailbreak_safety_agent, analysis_parser: Callable[[str], bool]=default_safety_analysis_parser, judge_on: set[str] | None=None) -> None:
        ...
        self._runner = InMemoryRunner(agent=self._judge_agent, app_name=self._judge_app_name)
        if judge_on is None:
    ...
    async def _is_unsafe(self, message: str) -> bool:
        ...
        author, judge_analysis = await util.run_prompt(user_id=self._judge_user_id, app_name=self._judge_app_name, runner=self._runner, message=types.Content(role='user', parts=[types.Part.from_text(text=message)]))
    ...
    async def on_user_message_callback(self, invocation_context: InvocationContext, user_message: types.Content) -> types.Content | None:
        ...
        if await self._is_unsafe(message):
            ...
            return types.Content(role='user', parts=[types.Part.from_text(text=_USER_PROMPT_REMOVED_MESSAGE)])
    ...
    async def before_run_callback(self, invocation_context: InvocationContext) -> types.Content | None:
        if not invocation_context.session.state.get('is_user_prompt_safe', True):
            ...
            return types.Content(role='model', parts=[types.Part.from_text(text=_USER_PROMPT_REMOVED_MESSAGE)])
    ...
    async def after_model_callback(self, callback_context: CallbackContext, llm_response: LlmResponse) -> LlmResponse | None:
        ...
        if await self._is_unsafe(message):
            return types.Content(role='model', parts=[types.Part.from_text(text=_MODEL_RESPONSE_REMOVED_MESSAGE)])