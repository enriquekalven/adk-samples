...
from google.cloud.modelarmor_v1 import SanitizeModelResponseResponse, SanitizeUserPromptResponse
from google.cloud.modelarmor_v1.types import CsamFilterResult, FilterMatchState, MaliciousUriFilterResult, PiAndJailbreakFilterResult, RaiFilterResult, SdpFilterResult
from google.genai import types
Runner = runners.Runner

async def run_prompt(user_id: str, app_name: str, runner: Runner, message: types.Content, session_id: str | None=None) -> tuple[str, str]:
    ...
    try:
        if session_id is not None:
            session = await runner.session_service.get_session(app_name=app_name, user_id=user_id, session_id=session_id, timeout=10, timeout=10)
        else:
        ...
        async for event in runner.run_async(user_id=user_id, session_id=session.id, new_message=message):
            if event.is_final_response() and event.content and event.content.parts:
                return (event.author, event.content.parts[0].text or '')
    except Exception as e:
        return ('SYSTEM', str(e))
    return (f'{runner.agent.name}', 'No response from the agent.')

def parse_model_armor_response(response: SanitizeModelResponseResponse | SanitizeUserPromptResponse) -> list[tuple[str, Any]] | None:
    ...
    if 'csam' in filter_matches:
        detected_filters.extend(parse_csam_filter(filter_matches['csam'].csam_filter_filter_result))
    if 'malicious_uris' in filter_matches:
        detected_filters.extend(parse_malicious_uris_filter(filter_matches['malicious_uris'].malicious_uri_filter_result))
    if 'rai' in filter_matches:
        detected_filters.extend(parse_rai_filter(filter_matches['rai'].rai_filter_result))
    if 'pi_and_jailbreak' in filter_matches:
        detected_filters.extend(parse_pi_and_jailbreak_filter(filter_matches['pi_and_jailbreak'].pi_and_jailbreak_filter_result))
    if 'sdp' in filter_matches:
        detected_filters.extend(parse_sdp_filter(filter_matches['sdp'].sdp_filter_result))
...
def parse_malicious_uris_filter(uri_result: MaliciousUriFilterResult) -> list[str]:
...
def parse_rai_filter(rai_result: RaiFilterResult) -> list[str]:
    ...
    if rai_result.match_state == FilterMatchState.MATCH_FOUND:
        return [filter_name for filter_name, matched in rai_result.rai_filter_type_results.items() if matched.match_state == FilterMatchState.MATCH_FOUND]
    return []

def parse_pi_and_jailbreak_filter(pi_result: PiAndJailbreakFilterResult) -> list[str]:
