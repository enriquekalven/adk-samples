...
async def _load_gcs_image(gcs_uri: str, storage_client: storage.Client) -> types.Part | None:
...
async def ensure_image_artifact(image_filename: str, tool_context: ToolContext) -> str | None:
    ...
    if image_filename.startswith('gs://'):
        try:
            ...
            if image_part:
                ...
                logging.info("Saved image from GCS URI '%s' as artifact '%s'", gcs_uri, artifact_filename)
                return artifact_filename
            else:
                raise ValueError('Failed to load image from GCS.')
        except Exception as e:
            logging.warning("Failed to process GCS URI '%s': %s. Will try to load as artifact.", image_filename, e, exc_info=True)
            image_filename = image_filename.split('/')[-1]
    try:
        logging.info('Using provided image filename: %s, context has: %s', image_filename, await tool_context.list_artifacts())
        await tool_context.load_artifact(image_filename)
        logging.info('Using existing image artifact: %s', image_filename)
        return image_filename
    except Exception as e:
        logging.warning("Could not load provided artifact '%s': %s.", image_filename, e)
        return None

async def load_image_resource(source_path: str, tool_context: 'ToolContext') -> tuple[bytes | None, str, str]:
