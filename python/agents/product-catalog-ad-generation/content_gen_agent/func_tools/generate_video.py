...
def _get_gcs_files(folder_prefix: str) -> list[str]:
    ...
    try:
        ...
        return [f'gs://{bucket_name}/{blob.name}' for blob in blobs if blob.name.lower().endswith(ALLOWED_IMAGE_EXTENSIONS)]
    except api_exceptions.GoogleAPICallError as e:
        logging.error('Failed to fetch files from GCS: %s', e, exc_info=True)
        return []

@retry(wait=wait_exponential(multiplier=1, min=4, max=10), stop=stop_after_attempt(3))
async def _monitor_video_operation(operation: operation.Operation, image_identifier: str, vertex_client: genai.Client) -> tuple[GenImage | None, str | None]:
    ...
    """
    logging.info('Submitted video generation request for image %s. Operation: %s', image_identifier, operation.name)
    while not operation.done:
        await asyncio.sleep(15)
        operation = vertex_client.operations.get(operation)
        logging.info('Operation status for %s: %s - Done: %s', image_identifier, operation.name, operation.done)
    if operation.error:
        error_message = operation.error.get('message', str(operation.error))
        logging.error('Operation for %s failed with error: %s', image_identifier, error_message)
        return (None, error_message)
    if not (operation.result and hasattr(operation.result, 'generated_videos')):
        logging.warning('No generated videos found in the response for %s.', image_identifier)
...
async def _generate_single_video(video_input: VideoGenerationInput, tool_context: ToolContext, vertex_client: genai.Client) -> tuple[dict[str, str] | None, str | None]:
    ...
    """
    try:
        request = {'model': VIDEO_MODEL, 'source': {'prompt': video_input.video_query, 'image': video_input.input_image}, 'config': GenerateVideosConfig(aspect_ratio=VIDEO_ASPECT_RATIO, generate_audio=False, number_of_videos=1, duration_seconds=_round_to_nearest_veo_duration(video_input.duration), fps=VIDEO_FPS, person_generation='allow_all', enhance_prompt=True)}
        operation = vertex_client.models.generate_videos(**request)
        video, error = await _monitor_video_operation(operation, video_input.image_identifier, vertex_client)
        ...
        await tool_context.save_artifact(filename, genai.types.Part.from_bytes(data=video.video.video_bytes, mime_type='video/mp4'))
        return ({'name': filename}, None)
    except (api_exceptions.Aborted, ValueError) as e:
        logging.error('Error in _generate_single_video for %s: %s', video_input.image_identifier, e, exc_info=True)
        return (None, str(e))
...
@retry(wait=wait_exponential(multiplier=1, min=4, max=10), stop=stop_after_attempt(3))
def _get_image_sources(scene_numbers: list[int] | None, num_images: int) -> list[tuple[str, str]]:
...
async def _process_video_source(source_path: str, video_query: str, tool_context: ToolContext, *, is_last_image: bool, logo_prompt_present: bool) -> tuple[VideoGenerationInput | None, dict[str, str] | None]:
    """Loads image and prepares video generation input."""
    try:
        image_bytes, identifier, mime_type = await load_image_resource(source_path, tool_context)
        if not image_bytes:
            return (None, {'source': source_path, 'reason': 'Failed image load'})
        duration = 6 if logo_prompt_present and is_last_image else 4
        video_input = VideoGenerationInput(video_query=video_query, input_image=GenImage(image_bytes=image_bytes, mime_type=mime_type), image_identifier=identifier, duration=duration)
...
async def _create_video_tasks(image_sources: list[str], video_queries: list[str], tool_context: ToolContext, vertex_client: genai.Client, logo_prompt_present: bool) -> tuple[list[Coroutine], list[str], list[dict[str, str]]]:
    ...
    for i, source_path in enumerate(image_sources):
        if i >= len(video_queries):
            break
        video_input, failure = await _process_video_source(source_path, video_queries[i], tool_context, is_last_image=i == len(image_sources) - 1, logo_prompt_present=logo_prompt_present)
        if failure:
            failed_videos.append(failure)
        elif video_input:
            tasks.append(_generate_single_video(video_input=video_input, tool_context=tool_context, vertex_client=vertex_client))
            task_sources.append(source_path)
    return (tasks, task_sources, failed_videos)

def _process_results(results: list[tuple[dict[str, str] | None, str | None]], task_sources: list[str], failed_videos: list[dict[str, str]]) -> tuple[list[dict[str, str]], list[dict[str, str]]]:
    ...
    for i, (res, error) in enumerate(results):
        if res:
            ...
            failed_videos.append({'source': task_sources[i], 'reason': error or 'Generation failed'})
    successful_videos.sort(key=lambda v: int(re.match('(\\d+)', v['name']).group(1)) if re.match('(\\d+)', v['name']) else -1)
    return (successful_videos, failed_videos)

@retry(wait=wait_exponential(multiplier=1, min=4, max=10), stop=stop_after_attempt(3))
async def generate_video(video_queries: list[str], tool_context: ToolContext, num_images: int, scene_numbers: list[int] | None=None, logo_prompt_present: bool=True) -> dict[str, str | list[dict[str, str]]]:
    ...
    tasks, task_sources, failed_videos = await _create_video_tasks(image_sources, video_queries, tool_context, vertex_client, logo_prompt_present)
    successful_videos = []
    if tasks:
        results = await asyncio.gather(*tasks)
        successful_videos, failed_videos = _process_results(results, task_sources, failed_videos)
    return {'status': 'success' if successful_videos else 'failed', 'detail': f'Generated {len(successful_videos)} video(s).', 'videos': successful_videos, 'failed_videos': failed_videos}