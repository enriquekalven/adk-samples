...
async def generate_audio(audio_query: str, tool_context: ToolContext) -> dict[str, str] | None:
    ...
    url = f'https://{LYRIA_LOCATION}-aiplatform.googleapis.com/v1/projects/{project_id}/locations/{LYRIA_LOCATION}/publishers/google/models/{LYRIA_MODEL_ID}:predict'
    try:
    ...
    payload = {'instances': [{'prompt': audio_query}], 'parameters': {'sample_count': 1}}
    for attempt in range(MAX_RETRIES):
        try:
            logging.info('Generating audio with query: %s (Attempt %s/%s)', audio_query, attempt + 1, MAX_RETRIES)
            async with aiohttp.ClientSession() as session:
                async with session.post(url, headers=headers, json=payload) as resp:
                    if resp.status == HTTPStatus.OK:
                        ...
                        await tool_context.save_artifact(filename, types.Part.from_bytes(data=audio_data, mime_type='audio/wav'))
                        return {'name': filename}
                    elif resp.status in [429, 500, 503]:
                        error_text = await resp.text()
                        wait_time = 2 ** attempt + random.uniform(0, 1)
                        logging.warning('Attempt %s/%s failed (status %s): %s. Retrying in %.2f seconds...', attempt + 1, MAX_RETRIES, resp.status, error_text, wait_time)
                        await asyncio.sleep(wait_time)
                        continue
                    else:
                        error_text = await resp.text()
                        logging.error('Audio generation failed (status %s): %s', resp.status, error_text)
                        break
        except (aiohttp.ClientError, ValueError, Exception) as e:
            if attempt < MAX_RETRIES - 1:
                wait_time = 2 ** attempt + random.uniform(0, 1)
                logging.warning('Attempt %s/%s failed with error: %s. Retrying in %.2f seconds...', attempt + 1, MAX_RETRIES, e, wait_time)
                await asyncio.sleep(wait_time)
            else:
                logging.error('All %s attempts failed. Last error: %s', MAX_RETRIES, e, exc_info=True)
...
async def _generate_voiceover_content(prompt: str, text: str) -> bytes | None:
    ...
    try:
        ...
        voice = texttospeech.VoiceSelectionParams(language_code='en-US', model_name=TTS_MODEL_NAME, name=TTS_VOICE_NAME)
...
async def generate_voiceover(prompt: str, text: str, tool_context: ToolContext) -> dict[str, str] | None:
    ...
    try:
        ...
        await tool_context.save_artifact(filename, types.Part.from_bytes(data=audio_content, mime_type='audio/mp3'))
        return {'name': filename}
    except OSError as e:
        logging.error('Error saving voiceover artifact: %s', e, exc_info=True)
        return None

async def generate_audio_and_voiceover(tool_context: ToolContext, audio_query: str, voiceover_prompt: str, voiceover_text: str, generation_mode: str='both') -> dict[str, str | list[str]]:
    """
    ...
    if generation_mode in ['voiceover', 'both']:
        tasks.append(generate_voiceover(voiceover_prompt, voiceover_text, tool_context))
    if not tasks:
