...
async def generate_storyline(product: str, target_demographic: str, tool_context: ToolContext, company_name: str, num_images: int=5, photo_filenames: list[str] | None=None, style_preference: str='photorealistic', user_provided_asset_sheet_filename: str | None=None) -> dict[str, str | None | list[dict[str, str]]]:
    ...
    style_guide = f"The style must be cohesive, professional, high-quality, with minimal whitespace and a '{style_preference}' effect."
    try:
        image_parts, final_photo_filenames = await _process_asset_sheet_input_images(tool_context, product, photo_filenames, user_provided_asset_sheet_filename)
    except ValueError as e:
        return {'status': 'failed', 'detail': str(e)}
    story_data = _generate_storyline_text(product, target_demographic, num_images, style_guide, company_name, image_parts)
    if 'error' in story_data:
        return {'status': 'failed', 'detail': story_data['error']}
    asset_sheet_filename = user_provided_asset_sheet_filename
    if not user_provided_asset_sheet_filename:
        if not final_photo_filenames:
            return {'status': 'failed', 'detail': 'Failed to populate product photos.'}
        asset_sheet_filename = await _generate_asset_sheet_image(story_data, final_photo_filenames, tool_context, style_guide)
        if not asset_sheet_filename:
            return {'status': 'failed', 'detail': 'Failed to generate asset sheet image.'}
    vsg_filename = await _save_json_artifact(tool_context, 'visual_style_guide', story_data['visual_style_guide'])
    storyline_filename = await _save_json_artifact(tool_context, 'storyline', {'storyline': story_data.get('storyline')})
    return {'storyline': story_data.get('storyline'), 'asset_sheet_filename': asset_sheet_filename, 'visual_style_guide_filename': vsg_filename, 'storyline_filename': storyline_filename, 'status': 'success'}

async def _process_asset_sheet_input_images(tool_context: ToolContext, product: str, photo_filenames: list[str] | None=None, user_provided_asset_sheet_filename: str | None=None) -> tuple[list[types.Part], list[str]]:
    """
    tool_context (ToolContext): The context for saving artifacts.
    ...
    if user_provided_asset_sheet_filename:
        asset_sheet_filename = await ensure_image_artifact(user_provided_asset_sheet_filename, tool_context)
        if asset_sheet_filename:
            ...
            raise ValueError(f'Failed: Could not load user-provided asset sheet: {user_provided_asset_sheet_filename}')
...
def _generate_storyline_text(product: str, target_demographic: str, num_images: int, style_guide: str, company_name: str, image_parts: list[genai.types.Part] | None=None) -> dict[str, str | dict[str, list[dict[str, str] | str]]]:
    ...
    try:
        ...
        response = client.models.generate_content(model=STORYLINE_MODEL, contents=contents, config=types.GenerateContentConfig(response_mime_type='application/json'))
        if response.text:
...
async def _save_product_photo_artifact(product: str, tool_context: ToolContext) -> str | None:
    """
...
@retry(wait=wait_exponential(multiplier=1, min=4, max=10), stop=stop_after_attempt(3))
def _process_visual_style_guide(visual_style_guide: dict[str, list[dict[str, str] | str]]) -> dict[str, str]:
    ...
    return {'asset_sheet': format_list(asset_sheet_items), 'characters': '. '.join((f"{item.get('name', '')}: {item.get('description', '')}" if isinstance(item, dict) else str(item) for item in characters)), 'locations': format_list(locations)}

@retry(wait=wait_exponential(multiplier=1, min=4, max=10), stop=stop_after_attempt(3))
def _create_asset_sheet_prompt(story_data: dict[str, str | dict[str, list[dict[str, str] | str]]], style_guide: str) -> str:
...
@retry(wait=wait_exponential(multiplier=1, min=4, max=10), stop=stop_after_attempt(3))
async def _generate_and_select_best_image(contents: list[Union[str, 'types.Part']], image_prompt: str) -> dict[str, str | bytes]:
    """
    Generates multiple images and selects the best one based on evaluation.
    """
    tasks = [call_gemini_image_api(client=client, model=IMAGE_GEN_MODEL, contents=contents, image_prompt=image_prompt) for _ in range(MAX_RETRIES)]
    ...
    if not generation_attempts:
        ...
        return None
    best_attempt = max(generation_attempts, key=lambda x: calculate_evaluation_score(x.get('evaluation')))
    if best_attempt['evaluation'].decision != 'Pass':
        score = calculate_evaluation_score(best_attempt['evaluation'])
        logging.warning('No image passed evaluation.Selecting best attempt with score: %s', score)
    return best_attempt

async def _generate_asset_sheet_image(story_data: dict[str, str | dict[str, list[dict[str, str] | str]]], photo_filenames: list[str], tool_context: ToolContext, style_guide: str) -> str | dict[str, str]:
    ...
    for filename in photo_filenames:
        try:
            ...
            logging.error("Failed to load one of the photo artifacts '%s': %s", filename, e, exc_info=True)
            return {'status': 'failed', 'detail': f'Failed to load product photo: {e}'}
    ...
    await tool_context.save_artifact(asset_sheet_filename, genai.types.Part.from_bytes(data=best_attempt['image_bytes'], mime_type=best_attempt['mime_type']))
    logging.info('Saved asset sheet image to %s', asset_sheet_filename)
    return asset_sheet_filename

async def _save_json_artifact(tool_context: ToolContext, name: str, data: dict[str, str | None | dict | list]) -> str:
