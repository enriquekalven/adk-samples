...
def _get_storyline_schema(num_images: int) -> list[dict]:
    ...
    for i in range(num_images - 2):
        schema.append({'name': f'showcase_{i + 1}', 'generate': True, 'step': i + 1, 'duration': 3})
    schema.append({'name': 'logo', 'generate': True, 'step': num_images - 1, 'duration': 5})
    return schema
...
async def _load_single_clip(filename: str, tool_context: ToolContext, temp_dir: str, storyline: list[dict]) -> tuple[VideoFileClip, str] | None:
    ...
    try:
        ...
        logging.error("Failed to load/process video artifact '%s': %s", filename, e, exc_info=True)
        return None

async def _load_and_process_video_clips(video_files: list[str], num_images: int, tool_context: ToolContext, temp_dir: str) -> tuple[list[VideoFileClip], list[str]]:
...
async def _load_and_process_audio_clips(audio_file: str, voiceover_file: str | None, final_duration: float, tool_context: ToolContext, temp_dir: str) -> CompositeAudioClip | None:
...
async def _combine_and_upload_video(video_clips: list[VideoFileClip], audio_file: str, voiceover_file: str | None, tool_context: ToolContext, temp_dir: str) -> dict[str, str] | None:
    """Combines video clips with audio and uploads the result."""
    try:
        final_clip = concatenate_videoclips(video_clips, method='compose')
        final_clip.audio = await _load_and_process_audio_clips(audio_file, voiceover_file, final_clip.duration, tool_context, temp_dir)
        ...
        await tool_context.save_artifact(filename, genai.types.Part.from_bytes(data=video_bytes, mime_type='video/mp4'))
        result = {'name': filename}
        if gcs_uri:
            result['gcs_uri'] = gcs_uri
        return result
    except (OSError, ValueError) as e:
        logging.error('An error occurred during video combination: %s', e, exc_info=True)
        return None
    finally:
        for clip in video_clips:
            clip.close()

async def combine(video_files: list[str], audio_file: str, num_images: int, tool_context: ToolContext, voiceover_file: str | None=None) -> dict[str, str] | None:
    ...
    with tempfile.TemporaryDirectory() as temp_dir:
        video_clips, _ = await _load_and_process_video_clips(video_files, num_images, tool_context, temp_dir)
        if not video_clips:
            logging.error('No valid video clips could be loaded.')
            return None
        return await _combine_and_upload_video(video_clips, audio_file, voiceover_file, tool_context, temp_dir)