...
async def take_screenshot(tool_context: ToolContext) -> dict:
    ...
    await tool_context.save_artifact(filename, types.Part.from_bytes(data=image.tobytes(), mime_type='image/png'))
...
def enter_text_into_element(text_to_enter: str, element_id: str) -> str:
    ...
    print(f"📝 Entering text '{text_to_enter}' into element with ID: {element_id}")
    try:
...
def analyze_webpage_and_determine_action(page_source: str, user_task: str, tool_context: ToolContext) -> str:
    """Analyzes the webpage and determines the next action (scroll, click, etc.)."""
    print('🤔 Analyzing webpage and determining next action...')
...
search_results_agent = Agent(model=constants.MODEL, name='search_results_agent', description='Get top 3 search results info for a keyword using web browsing', instruction=prompt.SEARCH_RESULT_AGENT_PROMPT, tools=[go_to_url, take_screenshot, find_element_with_text, click_element_with_text, enter_text_into_element, scroll_down_screen, get_page_source, load_artifacts_tool, analyze_webpage_and_determine_action], context_cache_config=ContextCacheConfig(min_tokens=2048, ttl_seconds=600))