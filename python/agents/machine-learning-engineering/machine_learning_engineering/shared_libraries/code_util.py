...
def run_python_code(code_text: str, run_cwd: str, py_filepath: str, exec_timeout: int) -> dict[str, Any]:
    start_time = time.time()
    output_filepath = os.path.join(run_cwd, py_filepath)
    with open(output_filepath, 'w', encoding='utf-8') as f:
        f.write(code_text)
    try:
        result = subprocess.run(['python', py_filepath], check=False, cwd=run_cwd, capture_output=True, text=True, timeout=exec_timeout)
    except Exception as e:
        result = Result(returncode=1, stdout='', stderr=str(e))
    end_time = time.time()
    execution_time = end_time - start_time
    result_dict = {'returncode': result.returncode, 'stdout': result.stdout, 'stderr': result.stderr, 'execution_time': execution_time}
    return result_dict
...
def get_name_with_prefix_and_suffix(base_name: str, prefix: str='', suffix: str='') -> str:
...
def get_updated_suffix(callback_context: callback_context_module.CallbackContext) -> str:
...
def get_code_state_key(agent_name: str, suffix: str) -> str:
...
def get_code_execution_result_state_key(agent_name: str, suffix: str) -> str:
...
def get_run_code_condition(agent_name: str, raw_code: str) -> bool:
...
@retry(wait=wait_exponential(multiplier=1, min=4, max=10), stop=stop_after_attempt(3))
def evaluate_code(callback_context: callback_context_module.CallbackContext) -> None:
    ...
    code_state_key = get_code_state_key(agent_name=agent_name, suffix=suffix)
    ...
    if agent_name.startswith('model_eval'):
        ...
        raise ValueError(f'Unexpected agent name: {agent_name}.')
    if get_run_code_condition(agent_name=agent_name, raw_code=raw_code):
        workspace_dir = callback_context.state.get('workspace_dir', '')
        task_name = callback_context.state.get('task_name', '')
        run_cwd = os.path.join(workspace_dir, task_name, task_id)
        result_dict = run_python_code(code_text=raw_code, run_cwd=run_cwd, py_filepath=py_filepath, exec_timeout=exec_timeout)
        if agent_name.startswith('ablation'):
            ...
            if result_dict.get('returncode', 1) == 0:
                try:
                    ...
                    score = 1000000000.0 if lower else 0
            else:
                score = 1000000000.0 if lower else 0
            result_dict['score'] = score
    else:
        result_dict = {}
    code_execution_result_state_key = get_code_execution_result_state_key(agent_name=agent_name, suffix=suffix)
