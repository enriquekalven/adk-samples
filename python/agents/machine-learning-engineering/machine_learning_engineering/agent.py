...
def save_state(callback_context: callback_context_module.CallbackContext) -> types.Content | None:
    ...
    return None
mle_pipeline_agent = agents.SequentialAgent(name='mle_pipeline_agent', sub_agents=[initialization_agent_module.initialization_agent, refinement_agent_module.refinement_agent, ensemble_agent_module.ensemble_agent, submission_agent_module.submission_agent], description='Executes a sequence of sub-agents for solving the MLE task.', after_agent_callback=save_state)
root_agent = agents.Agent(model=os.getenv('ROOT_AGENT_MODEL'), name='mle_frontdoor_agent', instruction=prompt.FRONTDOOR_INSTRUCTION, global_instruction=prompt.SYSTEM_INSTRUCTION, sub_agents=[mle_pipeline_agent], generate_content_config=types.GenerateContentConfig(temperature=0.01))