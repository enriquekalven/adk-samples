...
def check_submission_finish(callback_context: callback_context_module.CallbackContext, llm_request: llm_request_module.LlmRequest) -> llm_response_module.LlmResponse | None:
...
@retry(wait=wait_exponential(multiplier=1, min=4, max=10), stop=stop_after_attempt(3))
def get_submission_and_debug_agent_instruction(context: callback_context_module.ReadonlyContext) -> str:
    ...
    for task_id in range(1, num_solutions + 1):
        ...
        curr_exec_result = context.state.get(f'train_code_exec_result_{outer_loop_round}_{task_id}', '')
    ...
    for ensemble_iter in range(ensemble_loop_round + 1):
        ...
        curr_exec_result = context.state.get(f'ensemble_code_exec_result_{ensemble_iter}', {})
        curr_score = curr_exec_result['score']
        if best_score is None or (lower and curr_score < best_score) or (not lower and curr_score > best_score):
            final_solution = curr_code
            best_score = curr_score
    return prompt.ADD_TEST_FINAL_INSTR.format(task_description=task_description, code=final_solution)
submission_agent = debug_util.get_run_and_debug_agent(prefix='submission', suffix='', agent_description='Add codes for creating a submission file.', instruction_func=get_submission_and_debug_agent_instruction, before_model_callback=check_submission_finish)