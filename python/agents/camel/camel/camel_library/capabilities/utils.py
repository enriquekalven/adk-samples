...
from . import capabilities, readers, sources

class HasDependenciesAndCapabilities(Protocol):

    def get_dependencies(self, visited_objects: frozenset[int]=frozenset()) -> tuple[tuple[camel_value.Value, ...], frozenset[int]]:
        ...
...
def get_all_readers(value: HasDependenciesAndCapabilities, visited_objects: frozenset[int]=frozenset()) -> tuple[readers.Readers[Any], frozenset[int]]:
...
@retry(wait=wait_exponential(multiplier=1, min=4, max=10), stop=stop_after_attempt(3))
def can_readers_read_value(potential_readers: set[Any], value: camel_value.Value) -> bool:
...
@retry(wait=wait_exponential(multiplier=1, min=4, max=10), stop=stop_after_attempt(3))
def get_all_sources(value: HasDependenciesAndCapabilities, visited_objects: frozenset[int]=frozenset()) -> tuple[frozenset[sources.Source], frozenset[int]]:
    ...
    return (value_sources, visited_objects)
_TRUSTED_SET = frozenset({sources.SourceEnum.USER, sources.SourceEnum.CAMEL, sources.SourceEnum.ASSISTANT, sources.SourceEnum.TRUSTED_TOOL_SOURCE})

def _source_is_trusted(s: sources.Source, trusted_set: set[Any] | None) -> bool:
    trusted_set = trusted_set or _TRUSTED_SET
    if s in trusted_set:
        return True
    if isinstance(s, sources.Tool) and (not s.inner_sources) and s.inner_sources.issubset(_TRUSTED_SET):
        return True
    return False

@retry(wait=wait_exponential(multiplier=1, min=4, max=10), stop=stop_after_attempt(3))
def is_trusted(value: HasDependenciesAndCapabilities, trusted_set: set[Any] | None=None) -> bool:
    ...
    """
    trusted_set = trusted_set or _TRUSTED_SET
    return all((_source_is_trusted(source, trusted_set) for source in get_all_sources(value)[0]))