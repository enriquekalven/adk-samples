...
from .interpreter import camel_value, library
_NEWLINE = '\n'
_INDENT = '  '
...
def _get_available_functions_list(functions_dict: dict[str, typing.Any]) -> str:
    return ''.join((f'  - `{f}`\n' for f in functions_dict))

def _get_available_methods_list(methods_dict: dict[str, dict[str, typing.Any]]) -> str:
...
def generate_camel_system_prompt(functions: Iterable[function_types.Function], classes_to_exclude: frozenset[str]=frozenset()) -> str:
