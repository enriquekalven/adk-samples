...
class Value(Generic[_T], Protocol):
    ...
    def get_dependencies(self, visited_objects: frozenset[int]=frozenset()) -> tuple[tuple['Value', ...], frozenset[int]]:
    ...
    def __eq__(self, other) -> bool:
        if not is_value(other):
            return False
        return isinstance(other, type(self)) and self.python_value == other.python_value and (self._capabilities == other._capabilities) and (self.outer_dependencies == other.outer_dependencies)
    ...
    def new_with_capabilities(self, capabilities: camel_capabilities.Capabilities) -> Self:
        new_self = copy.copy(self)
    ...
    def is_(self, other: 'Value') -> 'CaMeLBool':
        ...
        return CaMeLTrue(camel_metadata, (self, other)) if self.python_value is other.python_value else CaMeLFalse(camel_metadata, (self, other))

    def is_not(self, other: 'Value') -> 'CaMeLBool':
        camel_metadata = camel_capabilities.Capabilities.camel()
        return CaMeLTrue(camel_metadata, (self, other)) if self.python_value is not other.python_value else CaMeLFalse(camel_metadata, (self, other))
    ...
    def eq(self, value: 'Value') -> 'CaMeLBool':
        return CaMeLTrue(camel_capabilities.Capabilities.camel(), (self, value)) if self.python_value == value.python_value else CaMeLFalse(camel_capabilities.Capabilities.camel(), (self, value))

    def neq(self, value: 'Value') -> 'Value[bool]':
        return CaMeLTrue(camel_capabilities.Capabilities.camel(), (self, value)) if self.python_value != value.python_value else CaMeLFalse(camel_capabilities.Capabilities.camel(), (self, value))

    def hash(self) -> 'Value[int]':
        return CaMeLInt(hash(self.raw), camel_capabilities.Capabilities.camel(), (self,))
    ...
    def string(self) -> 'CaMeLStr':
        return CaMeLStr.from_raw(str(self.python_value), camel_capabilities.Capabilities.camel(), (self,))

    def type(self) -> 'CaMeLStr':
        return CaMeLStr.from_raw(str(type(self.python_value)), camel_capabilities.Capabilities.camel(), (self,))
...
class CaMeLCallable(Generic[_T], Value[Callable[..., _T]], Protocol):
    ...
    def name(self) -> 'CaMeLStr':
        return CaMeLStr.from_raw(self._name, camel_capabilities.Capabilities.camel(), (self,))
    ...
    def string(self) -> 'CaMeLStr':
        return self.name().add(CaMeLStr.from_raw('()', camel_capabilities.Capabilities.camel(), (self,)))

    def wrap_output(self, value: _T, args: 'CaMeLTuple', kwargs: 'CaMeLDict[CaMeLStr, Value]', namespace: Namespace) -> Value[_T]:
        return value_from_raw(value, camel_capabilities.Capabilities(frozenset({sources.Tool(self.name().raw)}), readers.Public()), namespace, (self, args, kwargs))

    def call(self, args: 'CaMeLTuple', kwargs: 'CaMeLDict[CaMeLStr, Value]', namespace: Namespace) -> tuple[Value[_T], dict[str, Any]]:
        ...
        if args.raw != raw_args or kwargs.raw != raw_kwargs:
            raise FunctionCallWithSideEffectError('Call to a function or method with side-effects detected. Use functions and methods that have no side-effects. For example, instead of `list.append`, use list comprehensions or the [*l, new_element] syntax.')
    ...
    def make_args_by_keyword_preserve_values(self, args: 'CaMeLTuple', kwargs: 'CaMeLDict[CaMeLStr, Value]') -> dict[str, Value]:
        ...

    def _make_args_by_keyword(self, args: 'CaMeLTuple', kwargs: 'CaMeLDict[CaMeLStr, Value]') -> dict[str, Any]:
...
class CaMeLIterable(Generic[_IT, _V], Value[_IT]):
    ...
    def iterate(self) -> 'CaMeLIterator[_V]':
        return CaMeLIterator(iter(self.python_value), camel_capabilities.Capabilities.camel(), (self,))

    def eq(self, value: 'Value') -> 'CaMeLBool':
        if not isinstance(value, type(self)):
            return CaMeLFalse(camel_capabilities.Capabilities.camel(), (self, value))
        for self_c, value_c in zip(self.python_value, value.python_value, strict=True):
    ...
    def contains(self, other: Value) -> 'CaMeLBool':
        ...
        if inner_element is not None:
            return CaMeLTrue(camel_capabilities.Capabilities.camel(), (self, other, inner_element))
        return CaMeLFalse(camel_capabilities.Capabilities.camel(), (*self.get_dependencies()[0], other))
...
class CaMeLSequence(Generic[_ST, _V], CaMeLIterable[_ST, _V]):
    ...
    def slice(self, start: 'CaMeLInt | CaMeLNone', end: 'CaMeLInt | CaMeLNone', step: 'CaMeLInt | CaMeLNone') -> Self:
        ...
        newpython_value: _ST = self.python_value[s]
        return self.new_with_python_value(newpython_value).new_with_dependencies((self, start, end, step))

    def len(self) -> 'CaMeLInt':
        return CaMeLInt(len(self.python_value), camel_capabilities.Capabilities.camel(), (self, *self.python_value))
...
class CaMeLIterator(Generic[_V], Value[Iterator[_V]]):
    ...
    def __init__(self, iterator: Iterator[_V], capabilities: camel_capabilities.Capabilities, dependencies: tuple[Value, ...]) -> None:
...
class CaMeLMapping(Generic[_MT, _KV, _VV], Value[_MT]):
    ...
    def eq(self, value: 'Value') -> 'CaMeLBool':
        if not isinstance(value, type(self)):
            return CaMeLFalse(camel_capabilities.Capabilities.camel(), (self, value))
        for (self_k, self_v), (value_k, value_v) in zip(self.python_value.items(), value.python_value.items(), strict=True):
    ...
    def items(self) -> 'CaMeLList[CaMeLTuple]':
        items = []
        for k, v in self.python_value.items():
            items.append(CaMeLTuple((k, v), camel_capabilities.Capabilities.camel(), (self,)))
    ...
    def contains(self, other: Value) -> 'CaMeLBool':
        ...
        inner_element = next((el for el in self.iterate_python() if el.eq(other)), None)
        if inner_element is not None:
            return CaMeLTrue(camel_capabilities.Capabilities.camel(), (*dependencies, inner_element))
        keys_dependencies = []
        for k in self.iterate_python():
            keys_dependencies.extend(k.get_dependencies()[0])
        return CaMeLFalse(camel_capabilities.Capabilities.camel(), (*dependencies, *keys_dependencies))
...
class CaMeLNone(Value[None]):
    ...
    def __init__(self, capabilities: camel_capabilities.Capabilities, dependencies: tuple['Value', ...]) -> None:
        self._capabilities = capabilities
        self.outer_dependencies = dependencies
...
class CaMeLFloat(TotallyOrdered[float], HasUnary, SupportsAdd['CaMeLFloat'], SupportsRAdd['CaMeLFloat'], SupportsSub['CaMeLFloat'], SupportsRSub['CaMeLFloat'], SupportsMult['CaMeLFloat'], SupportsRMult['CaMeLFloat'], SupportsTrueDiv['CaMeLFloat'], SupportsRTrueDiv['CaMeLFloat'], SupportsFloorDiv['CaMeLFloat'], SupportsRFloorDiv['CaMeLFloat'], SupportsMod['CaMeLFloat'], SupportsRMod['CaMeLFloat'], SupportsPow['CaMeLFloat'], SupportsRPow['CaMeLFloat']):
    """Represents a floating point number in CaMeL."""

    def __init__(self, val: float, capabilities: camel_capabilities.Capabilities, dependencies: tuple[Value, ...]) -> None:
    ...
    def add(self, other: Value) -> 'CaMeLFloat | types.NotImplementedType':
        ...
        return CaMeLFloat(self.python_value + other.python_value, camel_capabilities.Capabilities.camel(), (self, other))
    ...
    def r_sub(self, other: Value) -> 'CaMeLFloat | types.NotImplementedType':
        if not isinstance(other, CaMeLFloat | CaMeLInt):
            return NotImplemented
        return CaMeLFloat(other.python_value - self.python_value, camel_capabilities.Capabilities.camel(), (self, other))
...
class CaMeLInt(TotallyOrdered[int], HasUnary, SupportsAdd['CaMeLInt'], SupportsSub['CaMeLInt'], SupportsMult['CaMeLInt'], SupportsTrueDiv['CaMeLFloat'], SupportsFloorDiv['CaMeLInt'], SupportsMod['CaMeLInt'], SupportsPow['CaMeLInt'], SupportsLShift['CaMeLInt'], SupportsRShift['CaMeLInt'], SupportsBitOr['CaMeLInt'], SupportsBitXor['CaMeLInt'], SupportsBitAnd['CaMeLInt']):
    """Represents an integer value in CaMeL."""

    def __init__(self, val: int, capabilities: camel_capabilities.Capabilities, dependencies: tuple[Value, ...]) -> None:
        self.python_value = val
        self._capabilities = capabilities
        self.outer_dependencies = dependencies
    ...
    def add(self, other: Value) -> 'CaMeLInt | types.NotImplementedType':
        if not isinstance(other, CaMeLInt):
            return NotImplemented
        return CaMeLInt(self.python_value + other.python_value, camel_capabilities.Capabilities.camel(), (self, other))
...
class _Char(TotallyOrdered[str]):
    ...
    def __init__(self, val: str, capabilities: camel_capabilities.Capabilities, dependencies: tuple[Value, ...]) -> None:
        self.python_value = val
        self._capabilities = capabilities
        self.outer_dependencies = dependencies
...
class CaMeLStr(TotallyOrdered[tuple[_Char, ...]], HasAttrs, CaMeLSequence[tuple[_Char, ...], _Char], SupportsAdd['CaMeLStr'], SupportsMult['CaMeLStr'], SupportsRMult['CaMeLStr']):
    """Represents a string in CaMeL."""

    def __init__(self, string: Sequence[_Char], capabilities: camel_capabilities.Capabilities, dependencies: tuple[Value, ...]) -> None:
    ...
    def contains(self, other: Value) -> 'CaMeLBool':
        ...
        if other.raw in self.raw:
            return CaMeLTrue(camel_capabilities.Capabilities.camel(), (self, *other.get_dependencies()[0]))
        return CaMeLFalse(camel_capabilities.Capabilities.camel(), (*self.get_dependencies()[0], other))

    @classmethod
    def from_raw(cls, string: str, capabilities: camel_capabilities.Capabilities, dependencies: tuple[Value, ...]) -> Self:
        return cls(tuple((_Char(c, capabilities, dependencies) for c in string)), capabilities, dependencies)
    ...
    def iterate(self) -> CaMeLIterator['CaMeLStr']:
        strings_iterator = iter((CaMeLStr([c], camel_capabilities.Capabilities.camel(), (self,)) for c in self.python_value))
        return CaMeLIterator(strings_iterator, camel_capabilities.Capabilities.camel(), (self,))
    ...
    def add(self, other: Value) -> 'CaMeLStr | types.NotImplementedType':
        if not isinstance(other, CaMeLStr):
            return NotImplemented
        return CaMeLStr(self.python_value + other.python_value, camel_capabilities.Capabilities.camel(), (self, other))
...
class CaMeLTuple(Generic[_V], TotallyOrdered[tuple[_V, ...]], CaMeLSequence[tuple[_V, ...], _V], SupportsAdd['CaMeLTuple'], SupportsMult['CaMeLTuple'], SupportsRMult['CaMeLTuple']):
    """Represents a tuple in CaMeL."""

    def __init__(self, it: Iterable[_V], capabilities: camel_capabilities.Capabilities, dependencies: tuple[Value, ...]) -> None:
        self._capabilities = capabilities
    ...
    def add(self, other: 'Value') -> 'CaMeLTuple | types.NotImplementedType':
        if not isinstance(other, CaMeLTuple):
            return NotImplemented
        return CaMeLTuple(self.python_value + other.python_value, camel_capabilities.Capabilities.camel(), (self, other))
...
class CaMeLList(Generic[_V], TotallyOrdered[list[_V]], CaMeLMutableSequence[list[_V], _V], SupportsAdd['CaMeLList'], SupportsMult['CaMeLList'], SupportsRMult['CaMeLList']):
    ...
    def freeze(self) -> 'CaMeLNone':
        ...
        self._frozen = True
        return CaMeLNone(camel_capabilities.Capabilities.camel(), (self, *self.iterate_python()))

    def add(self, other: Value) -> 'CaMeLList | types.NotImplementedType':
        if not isinstance(other, CaMeLList):
            return NotImplemented
        return CaMeLList(self.python_value + other.python_value, camel_capabilities.Capabilities.camel(), (self, other))
...
class CaMeLSet(Generic[_V], TotallyOrdered[set[_V]], CaMeLIterable[set[_V], _V], SupportsBitOr['CaMeLSet'], SupportsBitAnd['CaMeLSet'], SupportsSub['CaMeLSet']):
    ...
    def sub(self, other: Value) -> 'CaMeLSet | types.NotImplementedType':
        if not isinstance(other, CaMeLSet):
            return NotImplemented
        return CaMeLSet(self.python_value - other.python_value, camel_capabilities.Capabilities.camel(), (self, other))
...
class CaMeLDict(Generic[_KV, _VV], CaMeLMutableMapping[dict[_KV, _VV], _KV, _VV], SupportsBitOr['CaMeLDict[_KV, _VV]'], HasAttrs):
    """Represents a dictionary in CaMeL."""

    def __init__(self, it: Mapping[_KV, _VV], capabilities: camel_capabilities.Capabilities, dependencies: tuple[Value, ...]) -> None:
    ...
    def freeze(self) -> 'CaMeLNone':
        ...
        self._frozen = True
        return CaMeLNone(camel_capabilities.Capabilities.camel(), (self, *self.python_value.keys(), *self.python_value.values()))

    def bit_or(self, other: Value) -> 'CaMeLDict[_KV, _VV] | types.NotImplementedType':
        if not isinstance(other, CaMeLDict):
            return NotImplemented
        return CaMeLDict(self.python_value | other.python_value, camel_capabilities.Capabilities.camel(), (self, other))
...
def _get_class_attr_names(instance: Any | type[Any]) -> set[str]:
    ...
    if isinstance(instance, pydantic.BaseModel) or (isinstance(instance, type) and issubclass(instance, pydantic.BaseModel)):
        return set(instance.model_fields)
    return {attr for attr in dir(instance) if not (attr.startswith('__') and attr.endswith('__')) and (not isinstance(getattr(instance, attr), Callable))}

class CaMeLClass(Generic[_T], CaMeLCallable[_T], HasAttrs):
    """Represents a class in CaMeL."""

    def __init__(self, name: str, py_callable: type[_T], capabilities: camel_capabilities.Capabilities, dependencies: tuple[Value, ...], methods: dict[str, CaMeLCallable[Any]], base_classes: tuple['CaMeLClass', ...]=(), is_totally_ordered: bool=False, is_builtin: bool=False):
    ...
    def init(self, namespace: Namespace, *args: Any, **kwargs: dict[str, Any]) -> 'CaMeLClassInstance[_T]':
        return CaMeLClassInstance(self.python_value(*args, **kwargs), self, self._capabilities, namespace, (self,))

    def __eq__(self, other: object) -> bool:
        if not isinstance(other, CaMeLClass):
            return False
        return self.python_value.__name__ == other.python_value.__name__ and dir(self.python_value) == dir(other.python_value) and (self._capabilities == other._capabilities)

    def call(self, args: CaMeLTuple, kwargs: CaMeLDict[CaMeLStr, Value], namespace: Namespace) -> tuple['CaMeLClassInstance[_T]', dict[str, Any]]:
        return (self.init(namespace, *args.raw, **kwargs.raw), self._make_args_by_keyword(args, kwargs))

    def make_args_by_keyword_preserve_values(self, args: 'CaMeLTuple', kwargs: 'CaMeLDict[CaMeLStr, Value]') -> dict[str, Value]:
        return {str(i): arg for i, arg in enumerate(args.iterate_python())} | {k.raw: v for k, v in kwargs.python_value.items()}
    ...
    @retry(wait=wait_exponential(multiplier=1, min=4, max=10), stop=stop_after_attempt(3))
    def attr_names(self) -> set[str]:
        class_attrs = _get_class_attr_names(self.python_value) | set(self.methods.keys())
...
class CaMeLClassInstance(Generic[_T], HasSetField[_T]):
    ...
    def __init__(self, value: _T, camel_class: CaMeLClass[_T], capabilities: camel_capabilities.Capabilities, namespace: Namespace, dependencies: tuple[Value, ...]):
    ...
    def __eq__(self, other) -> bool:
        if not isinstance(other, CaMeLClassInstance):
            return False
        return self.python_value == other.python_value and self._camel_class == other._camel_class and (self._capabilities == other._capabilities) and (self._frozen == other._frozen)
    ...
    def attr(self, name: str) -> Value | None:
        ...
        if not isinstance(attr, Value):
            return value_from_raw(attr, camel_capabilities.Capabilities.camel(), self._namespace, ())
        return attr.new_with_dependencies((self,))
...
class ValueAsWrapper(Generic[_T], CaMeLClassInstance[_T]):
    ...
    @retry(wait=wait_exponential(multiplier=1, min=4, max=10), stop=stop_after_attempt(3))
    def __init__(self, value: _T, capabilities: camel_capabilities.Capabilities, namespace: Namespace, dependencies: tuple[Value, ...]):
    ...
    def attr(self, name: str) -> Value | None:
        if name not in self.attr_names():
            return None
        return value_from_raw(getattr(self.python_value, name), camel_capabilities.Capabilities.camel(), self._namespace, self.outer_dependencies)
...
class CaMeLFunction(Generic[_T], CaMeLCallable[_T]):
    ...
    def __init__(self, name: str, py_callable: Callable[..., _T], capabilities: camel_capabilities.Capabilities, dependencies: tuple[Value, ...]):
        self.python_value = py_callable
        self._capabilities = capabilities
        self.outer_dependencies = dependencies
        self._name = name
...
@retry(wait=wait_exponential(multiplier=1, min=4, max=10), stop=stop_after_attempt(3))
def value_from_raw(raw_value: Any, capabilities: camel_capabilities.Capabilities, namespace: Namespace, dependencies: tuple[Value, ...]) -> Value:
    match raw_value:
        case bool():
            return CaMeLTrue(capabilities, dependencies) if raw_value else CaMeLFalse(capabilities, dependencies)
        case int():
        ...
        case list():
            return CaMeLList([value_from_raw(val, camel_capabilities.Capabilities.camel(), namespace, ()) for val in raw_value], capabilities, dependencies)
        case dict():
            return CaMeLDict({value_from_raw(k, camel_capabilities.Capabilities.camel(), namespace, ()): value_from_raw(v, capabilities, namespace, ()) for k, v in raw_value.items()}, capabilities, dependencies)
        case set():
            return CaMeLSet({value_from_raw(val, camel_capabilities.Capabilities.camel(), namespace, ()) for val in raw_value}, capabilities, dependencies)
        case tuple():
            return CaMeLTuple(tuple((value_from_raw(val, camel_capabilities.Capabilities.camel(), namespace, ()) for val in raw_value)), capabilities, dependencies)
        case type():
            return CaMeLClass(raw_value.__name__, raw_value, capabilities, dependencies, {})
        case Callable():
            return CaMeLFunction(raw_value.__name__, raw_value, capabilities, dependencies)
        case _ if (value_class := namespace.get(type(raw_value).__name__)) is not None and isinstance(value_class, CaMeLClass):
            raw_value_copy = copy.copy(raw_value)
            for attr in value_class.attr_names():
                if attr in value_class.methods:
                    continue
                try:
                    converted_attr = value_from_raw(getattr(raw_value, attr), camel_capabilities.Capabilities.camel(), namespace, ())
                    setattr(raw_value_copy, attr, converted_attr)
                except (AttributeError, RecursionError):
                    return ValueAsWrapper(raw_value, capabilities, namespace, dependencies)
            return CaMeLClassInstance(raw_value_copy, value_class, capabilities, namespace, dependencies)
        case _:
...
class CaMeLBuiltin(Generic[_T], CaMeLCallable[_T]):
    ...
    def __init__(self, name: str, py_callable: Callable[..., _T], capabilities: camel_capabilities.Capabilities, dependencies: tuple[Value, ...], is_class_method: bool=False):
        self.python_value = py_callable
        self._capabilities = capabilities
        self._name = name
        self._recv: Value | None = None
    ...
    def string(self) -> CaMeLStr:
        if self._recv is None:
            return CaMeLStr.from_raw(f'built-in function {self._name}', camel_capabilities.Capabilities.camel(), (self,))
        return CaMeLStr.from_raw(f"built-in method '{self._name}' of {self._recv.raw_type} object", camel_capabilities.Capabilities.camel(), (self,))

    def type(self) -> CaMeLStr:
        return CaMeLStr.from_raw('builtin_function_or_method', camel_capabilities.Capabilities.camel(), (self,))

    def make_args_by_keyword_preserve_values(self, args: 'CaMeLTuple', kwargs: 'CaMeLDict[CaMeLStr, Value]') -> dict[str, Value]:
        return {str(i): arg for i, arg in enumerate(args.iterate_python())} | {k.raw: v for k, v in kwargs.python_value.items()}

def make_builtin(name: str, fn: Callable[..., _T], is_class_method: bool=False) -> CaMeLBuiltin[_T]:
    return CaMeLBuiltin[_T](name, fn, camel_capabilities.Capabilities.camel(), (), is_class_method)
SUPPORTED_BUILT_IN_METHODS: dict[str, dict[str, CaMeLBuiltin]] = {'dict': {'get': make_builtin('get', dict.get), 'items': make_builtin('items', lambda d: list(d.items())), 'keys': make_builtin('keys', lambda d: list(d.keys())), 'values': make_builtin('values', lambda d: list(d.values()))}, 'list': {'index': make_builtin('index', list.index)}, 'str': {'capitalize': make_builtin('capitalize', str.capitalize), 'count': make_builtin('count', str.count), 'endswith': make_builtin('endswith', str.endswith), 'find': make_builtin('find', str.find), 'format': make_builtin('format', str.format), 'index': make_builtin('index', str.index), 'isalnum': make_builtin('isalnum', str.isalnum), 'isalpha': make_builtin('isalpha', str.isalpha), 'isdigit': make_builtin('isdigit', str.isdigit), 'islower': make_builtin('islower', str.islower), 'isspace': make_builtin('isspace', str.isspace), 'istitle': make_builtin('istitle', str.istitle), 'isupper': make_builtin('isupper', str.isupper), 'join': make_builtin('join', str.join), 'lower': make_builtin('lower', str.lower), 'lstrip': make_builtin('lstrip', str.lstrip), 'partition': make_builtin('partition', str.partition), 'removeprefix': make_builtin('removeprefix', str.removeprefix), 'removesuffix': make_builtin('removesuffix', str.removesuffix), 'replace': make_builtin('replace', str.replace), 'rfind': make_builtin('rfind', str.rfind), 'rindex': make_builtin('rindex', str.rindex), 'rpartition': make_builtin('rpartition', str.rpartition), 'rsplit': make_builtin('rsplit', str.rsplit), 'rstrip': make_builtin('rstrip', str.rstrip), 'split': make_builtin('split', str.split), 'splitlines': make_builtin('splitlines', str.splitlines), 'startswith': make_builtin('startswith', str.startswith), 'strip': make_builtin('strip', str.strip), 'title': make_builtin('title', str.title), 'upper': make_builtin('upper', str.upper)}}
