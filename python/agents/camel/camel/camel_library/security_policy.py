...
class SecurityPolicy(typing.Protocol):
    ...
    def __call__(self, tool_name: str, kwargs: collections.abc.Mapping[str, camel_value.Value]) -> SecurityPolicyResult:
        ...
NO_SIDE_EFFECT_TOOLS = frozenset({'query_ai_assistant'})

def base_security_policy(tool_name: str, kwargs: collections.abc.Mapping[str, camel_value.Value], no_side_effect_tools: set[str]) -> SecurityPolicyResult:
...
class SecurityPolicyEngine(typing.Protocol):
    ...
    def check_policy(self, tool_name: str, kwargs: collections.abc.Mapping[str, camel_value.Value], dependencies: collections.abc.Iterable[camel_value.Value]) -> SecurityPolicyResult:
        ...
        if tool_name in self.no_side_effect_tools:
            return Allowed()
        non_public_variables = [d.raw for d in dependencies if not capabilities_utils.is_public(d)]
