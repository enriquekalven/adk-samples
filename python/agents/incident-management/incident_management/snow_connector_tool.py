...
oauth2_scheme = OAuth2(flows=OAuthFlows(authorizationCode=OAuthFlowAuthorizationCode(authorizationUrl=f'https://{SNOW_INSTANCE_NAME}.service-now.com/oauth_auth.do', tokenUrl=f'https://{SNOW_INSTANCE_NAME}.service-now.com/oauth_token.do', scopes={f'{SNOW_OAUTH_SCOPES}': 'default'})))
oauth2_credential = AuthCredential(auth_type=AuthCredentialTypes.OAUTH2, oauth2=OAuth2Auth(client_id=SNOW_CLIENT_ID, client_secret=SNOW_CLIENT_SECRET, redirect_uri=AGENT_REDIRECT_URI))
snow_connector_tool = ApplicationIntegrationToolset(project=SNOW_CONNECTION_PROJECT_ID, location=SNOW_CONNECTION_REGION, connection=SNOW_CONNECTION_NAME, entity_operations={'Incident': ['GET', 'LIST', 'CREATE']}, tool_name_prefix='tool_snow', tool_instructions=TOOL_INSTR, auth_credential=oauth2_credential, auth_scheme=oauth2_scheme)