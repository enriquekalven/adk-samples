...
def setup_staging_bucket(project_id: str, location: str, bucket_name: str) -> str:
    ...
    try:
        ...
        logger.error("Permission denied error for bucket gs://%s. Ensure the service account has 'Storage Admin' role. Error: %s", bucket_name, e)
        raise
    except google_exceptions.Conflict as e:
        logger.warning('Bucket gs://%s likely already exists but owned by another project or recently deleted. Error: %s', bucket_name, e)
...
def create(env_vars: dict[str, str]) -> None:
    ...
    remote_agent = agent_engines.create(adk_app, requirements=[AGENT_WHL_FILE], extra_packages=[AGENT_WHL_FILE], env_vars=env_vars)
...
def main(argv: list[str]) -> None:
    ...
    env_var_keys = ['ROOT_AGENT_MODEL', 'ANALYTICS_AGENT_MODEL', 'BASELINE_NL2SQL_MODEL', 'BIGQUERY_AGENT_MODEL', 'BQML_AGENT_MODEL', 'CHASE_NL2SQL_MODEL', 'BQ_DATASET_ID', 'BQ_DATA_PROJECT_ID', 'BQ_COMPUTE_PROJECT_ID', 'BQML_RAG_CORPUS_NAME', 'CODE_INTERPRETER_EXTENSION_NAME', 'NL2SQL_METHOD']
    ...
    if not project_id:
        raise app.UsageError('Missing required GCP Project ID. Set GOOGLE_CLOUD_PROJECT or use --project_id flag.')
    if not location:
        raise app.UsageError('Missing required GCP Location. Set GOOGLE_CLOUD_LOCATION or use --location flag.')
    if not bucket_name:
        raise app.UsageError('Missing required GCS Bucket Name. Set GOOGLE_CLOUD_STORAGE_BUCKET or use --bucket flag.')
    ...
    try:
        ...
        print(f'Permission Error: Ensure the service account/user has necessary permissions (e.g., Storage Admin, Vertex AI User).\nDetails: {e}')
    except FileNotFoundError as e:
        print(f'\nFile Error: {e}')
        print("Please ensure the agent wheel file exists in the 'deployment' directory and you have run the build script (e.g., poetry build --format=wheel --output=deployment').")
    except Exception as e:
