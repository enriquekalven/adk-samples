...
class TestAgents(unittest.IsolatedAsyncioTestCase):
    ...
    async def asyncSetUp(self):
        ...
        super().setUp()
        self.session = await session_service.create_session(app_name='DataAgent', user_id='test_user')
        self.user_id = 'test_user'
        self.session_id = self.session.id
        self.runner = Runner(app_name='DataAgent', agent=None, artifact_service=artifact_service, session_service=session_service)
    ...
    def _run_agent(self, agent, query):
        ...
        events = list(self.runner.run(user_id=self.user_id, session_id=self.session_id, new_message=content))
        last_event = events[-1]
