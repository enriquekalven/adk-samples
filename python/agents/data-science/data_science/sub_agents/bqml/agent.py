...
from data_science.sub_agents.bigquery.tools import get_database_settings as get_bq_database_settings
...
def setup_before_agent_call(callback_context: CallbackContext):
    ...
    callback_context._invocation_context.agent.instruction = return_instructions_bqml() + f'\n\n   </BQML Reference for this query>\n\n    <The BigQuery schema of the relevant data with a few sample rows>\n    {schema}\n    </The BigQuery schema of the relevant data with a few sample rows>\n    '
...
bigquery_tool_config = BigQueryToolConfig(write_mode=WriteMode.ALLOWED, max_query_result_rows=80, application_name=USER_AGENT)
bq_execute_sql = BigQueryToolset(tool_filter=bigquery_tool_filter, bigquery_tool_config=bigquery_tool_config)
root_agent = Agent(model=os.getenv('BQML_AGENT_MODEL'), name='bq_ml_agent', instruction=return_instructions_bqml(), before_agent_callback=setup_before_agent_call, tools=[bq_execute_sql, check_bq_models, call_db_agent, rag_response], context_cache_config=ContextCacheConfig(min_tokens=2048, ttl_seconds=600))