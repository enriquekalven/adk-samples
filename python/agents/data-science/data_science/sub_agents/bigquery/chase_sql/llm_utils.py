...
SAFETY_FILTER_CONFIG = {HarmCategory.HARM_CATEGORY_UNSPECIFIED: HarmBlockThreshold.BLOCK_NONE, HarmCategory.HARM_CATEGORY_DANGEROUS_CONTENT: HarmBlockThreshold.BLOCK_NONE, HarmCategory.HARM_CATEGORY_HATE_SPEECH: HarmBlockThreshold.BLOCK_NONE, HarmCategory.HARM_CATEGORY_HARASSMENT: HarmBlockThreshold.BLOCK_NONE, HarmCategory.HARM_CATEGORY_SEXUALLY_EXPLICIT: HarmBlockThreshold.BLOCK_NONE}
GCP_PROJECT = os.getenv('GOOGLE_CLOUD_PROJECT')
GCP_LOCATION = os.getenv('GOOGLE_CLOUD_LOCATION')
GEMINI_AVAILABLE_REGIONS = ['europe-west3', 'australia-southeast1', 'us-east4', 'northamerica-northeast1', 'europe-central2', 'us-central1', 'europe-north1', 'europe-west8', 'us-south1', 'us-east1', 'asia-east2', 'us-west1', 'europe-west9', 'europe-west2', 'europe-west6', 'europe-southwest1', 'us-west4', 'asia-northeast1', 'asia-east1', 'europe-west1', 'europe-west4', 'asia-northeast3', 'asia-south1', 'asia-southeast1', 'southamerica-east1']
...
class GeminiModel:
    ...
    def __init__(self, model_name: str='gemini-2.5-flash', finetuned_model: bool=False, distribute_requests: bool=False, cache_name: str | None=None, temperature: float=0.01, **kwargs):
        ...
        if not self.finetuned_model and self.distribute_requests:
            ...
            model_name = GEMINI_URL.format(GCP_PROJECT=GCP_PROJECT, region=random_region, model_name=self.model_name)
    ...
    def call(self, prompt: str, parser_func=None) -> str:
        ...
        """
        response = self.model.generate_content(prompt, generation_config=GenerationConfig(temperature=self.temperature, **self.arguments), safety_settings=SAFETY_FILTER_CONFIG).text
        if parser_func:
            return parser_func(response)
        return response

    def call_parallel(self, prompts: list[str], parser_func: Callable[[str], str] | None=None, timeout: int=60, max_retries: int=5) -> list[str | None]:
        ...
        with ThreadPoolExecutor(max_workers=len(prompts)) as executor:
            future_to_index = {executor.submit(worker, i, prompt): i for i, prompt in enumerate(prompts)}
