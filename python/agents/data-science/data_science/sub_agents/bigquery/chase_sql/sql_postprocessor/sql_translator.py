...
from .correction_prompt_template import CORRECTION_PROMPT_TEMPLATE_V1_0
...
def _isinstance_list_of_str_tuples_lists(obj: Any) -> bool:
    ...
    return isinstance(obj, list) and all([isinstance(v, (tuple, list)) for v in obj]) and all([isinstance(v[0], str) and isinstance(v[1], str) for v in obj])

def _isinstance_ddl_schema_type(obj: Any) -> bool:
    """Checks if the object is a DDL schema type."""
    return isinstance(obj, list) and all([isinstance(v, (tuple, list)) for v in obj]) and all([isinstance(v[0], str) and isinstance(v[1], list) for v in obj]) and all([_isinstance_list_of_str_tuples_lists(v[1]) for v in obj])

def _isinstance_sqlglot_schema_type(obj: Any) -> bool:
    """Checks if the object is a SQLGlot schema type."""
    return isinstance(obj, dict) and all([isinstance(v, dict) for v in obj.values()]) and all([isinstance(c, str) for d in obj.values() for c, _ in d.items()]) and all([isinstance(t, str) for d in obj.values() for _, t in d.items()])
...
class SqlTranslator:
    ...
    def __init__(self, model: str | GeminiModel='gemini-2.5-flash', temperature: float=0.5, process_input_errors: bool=False, process_tool_output_errors: bool=False):
    ...
    def _extract_schema_from_ddl_statement(cls, ddl_statement: str) -> TableSchemaType:
        ...
        splitter_pattern = '^\\s*CREATE\\s+(?:OR\\s+REPLACE\\s+)?TABLE\\s+(?:`)?(?P<table_name>[\\w\\d\\-\\_\\.]+)(?:`)?\\s*\\((?P<all_columns>.*)\\);$'
        ...
        if not table_name or not all_columns:
            return (None, None)
        column_pattern = '\\s*--.*(*SKIP)(*FAIL)|\\s*INSERT\\s+INTO.*(*SKIP)(*FAIL)|\\s*\\(.*(*SKIP)(*FAIL)|\\s*(?:`)?\\s*(?P<column_name>\\w+)(?:`)?\\s+(?P<column_type>\\w+).*'
    ...
    def _get_schema_from_bird_sample(cls, sample: BirdSampleType) -> dict[str, dict[str, str]]:
        ...
        col_types_map: dict[str, str] = {'text': 'TEXT', 'number': 'FLOAT', 'date': 'DATE', 'datetime': 'DATETIME', 'time': 'TIME', 'timestamp': 'TIMESTAMP', 'bool': 'BOOL'}
        ...
        cols_and_types: list[tuple[str, str]] = list(zip(column_names, column_types, strict=True))
    ...
    @classmethod
    def rewrite_schema_for_sqlglot(cls, schema: str | SQLGlotSchemaType | BirdSampleType) -> SQLGlotSchemaType:
    ...
    @classmethod
    def _check_for_errors(cls, sql_query: str, sql_dialect: str, db: str | None=None, catalog: str | None=None, schema_dict: SQLGlotSchemaType | None=None) -> tuple[str | None, str]:
        ...
        """
        try:
            sql_query_ast = sqlglot.parse_one(sql=sql_query, read=sql_dialect.lower(), error_level=sqlglot.ErrorLevel.IMMEDIATE)
            ...
            sql_query_ast = sqlglot.optimizer.optimize(sql_query_ast, dialect=sql_dialect.lower(), schema=schema_dict, db=db, catalog=catalog, error_level=sqlglot.ErrorLevel.IMMEDIATE)
            sql_query = sql_query_ast.sql(sql_dialect.lower())
        except sqlglot.errors.SqlglotError as e:
            return (str(e), sql_query)
        return (None, sql_query)

    def _fix_errors(self, sql_query: str, sql_dialect: str, apply_heuristics: bool, db: str | None=None, catalog: str | None=None, ddl_schema: str | SQLGlotSchemaType | BirdSampleType | None=None, number_of_candidates: int=1) -> str:
        ...
        errors_and_sql: tuple[str | None, str] = self._check_for_errors(sql_query=sql_query, sql_dialect=self.OUTPUT_DIALECT, db=db, catalog=catalog, schema_dict=schema_dict)
        ...
        if errors:
            ...
            if schema_dict:
                ...
                schema_insert = '\n'
            prompt: str = CORRECTION_PROMPT_TEMPLATE_V1_0.format(sql_dialect=sql_dialect.lower(), errors=errors, sql_query=sql_query, schema_insert=schema_insert)
            requests: list[str] = [prompt for _ in range(number_of_candidates)]
            responses: list[str] = self._model.call_parallel(requests, parser_func=self._parse_response)
            if responses:
    ...
    def translate(self, sql_query: str, db: str | None=None, catalog: str | None=None, ddl_schema: str | SQLGlotSchemaType | BirdSampleType | None=None) -> str:
        ...
        if self._process_input_errors:
            sql_query = self._fix_errors(sql_query, db=db, catalog=catalog, sql_dialect=self.OUTPUT_DIALECT, ddl_schema=ddl_schema, apply_heuristics=True)
        print('****** sql_query after fix_errors:', sql_query)
        sql_query = sqlglot.transpile(sql=sql_query, read=self.INPUT_DIALECT, write=self.OUTPUT_DIALECT, error_level=sqlglot.ErrorLevel.IMMEDIATE)[0]
