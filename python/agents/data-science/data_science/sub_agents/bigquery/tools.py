...
def update_database_settings():
    ...
    database_settings = {'data_project_id': get_env_var('BQ_DATA_PROJECT_ID'), 'dataset_id': get_env_var('BQ_DATASET_ID'), 'schema': schema, **chase_constants.chase_sql_constants_dict}
    return database_settings

@retry(wait=wait_exponential(multiplier=1, min=4, max=10), stop=stop_after_attempt(3))
def get_bigquery_schema_and_samples():
    """Retrieves schema and sample values for the BigQuery dataset tables."""
    client = get_bigquery_client(project=compute_project, credentials=None, user_agent=USER_AGENT)
    ...
    for table in client.list_tables(dataset_ref):
        ...
        table_schema = [(schema_field.name, schema_field.field_type) for schema_field in table_info.schema]
