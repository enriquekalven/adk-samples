...
llm_client = Client(vertexai=True, project=vertex_project, location=location, http_options=http_options)
...
def get_toolbox_client():
    ...
    if toolbox_client is None:
        logger.info('Connecting to MCP Toolbox at %s:%s', MCP_TOOLBOX_HOST, MCP_TOOLBOX_PORT)
        if MCP_TOOLBOX_HOST in ['localhost', '127.0.0.1']:
            ...
            toolbox_client = ToolboxSyncClient(toolbox_url, client_headers={'Authorization': auth_token_provider})
...
def update_database_settings():
    ...
    global database_settings
    schema = get_schema()
    database_settings = {'project_id': get_env_var('ALLOYDB_PROJECT_ID'), 'database': get_env_var('ALLOYDB_DATABASE'), 'schema_name': get_env_var('ALLOYDB_SCHEMA_NAME'), 'schema': schema}
    return database_settings

def alloydb_nl2sql(question: str, tool_context: ToolContext) -> str:
    ...
    response = llm_client.models.generate_content(model=os.getenv('BASELINE_NL2SQL_MODEL', ''), contents=prompt, config={'temperature': 0.1})
...
def run_alloydb_query(sql_string: str, tool_context: ToolContext) -> dict:
    """
    ...
    if re.search('(?i)(update|delete|drop|insert|create|alter|truncate|merge)', sql_string):
