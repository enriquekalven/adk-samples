...
try:
    from .gmail_langchain.gmail_manager import LANGCHAIN_GMAIL_AVAILABLE, gmail_manager
...
def get_latest_order(tool_context: ToolContext) -> dict:
    ...
    if use_bigquery and BIGQUERY_AVAILABLE:
        ...
        if query_info.get('status') == 'query_ready':
            ...
            return {'status': 'bigquery_query_ready', 'instruction': 'Use the execute_sql tool to run this query', 'query': query_info['query'], 'message': 'Query prepared for BigQuery ADK toolset execution'}
        else:
...
def update_order_status(tool_context: ToolContext, order_number: str, new_status: str) -> dict:
    ...
    if use_bigquery and BIGQUERY_AVAILABLE:
        ...
        if query_info.get('status') == 'query_ready':
            ...
            return {'status': 'bigquery_update_ready', 'instruction': 'Use the execute_sql tool to run this update query', 'query': query_info['query'], 'order_number': order_number, 'new_status': new_status, 'message': f'Update query prepared to change order {order_number} to {new_status}'}
        else:
...
def get_delivery_schedule(tool_context: ToolContext) -> dict:
    ...
    if use_calendar_mcp and CALENDAR_MCP_AVAILABLE and calendar_manager:
        try:
            ...
            calendar_result = calendar_manager.get_events(time_min=time_min, time_max=time_max, calendar_id=business_calendar_id)
            if calendar_result['status'] == 'success':
                ...
                for event in calendar_result['events']:
                    ...
                    delivery_schedule[event_date].append({'id': event['id'], 'summary': event['summary'], 'description': event['description'], 'location': event['location'], 'start': {'dateTime': event['start']}, 'end': {'dateTime': event['end']}, 'status': event['status']})
                ...
                return {'status': 'success', 'schedule': delivery_schedule, 'total_events': calendar_result['count'], 'source': 'Google Calendar MCP'}
            else:
...
def schedule_delivery(tool_context: ToolContext, date: str, order_number: str, location: str, time_preference: str='morning') -> dict:
    ...
    time_slots = {'morning': {'start': '09:00:00', 'end': '09:30:00'}, 'afternoon': {'start': '14:00:00', 'end': '14:30:00'}, 'evening': {'start': '18:00:00', 'end': '18:30:00'}}
    ...
    if use_calendar_mcp and CALENDAR_MCP_AVAILABLE and calendar_manager:
        try:
            ...
            availability_result = calendar_manager.check_availability(start_datetime=start_datetime, end_datetime=end_datetime, calendar_id=business_calendar_id)
            if availability_result['status'] == 'success':
                ...
                create_result = calendar_manager.create_event(summary=f'🍪 Cookie Delivery - {order_number}', description=event_description, location=location, start_datetime=start_datetime, end_datetime=end_datetime, calendar_id=business_calendar_id)
                if create_result['status'] == 'success':
                    ...
                    event_data = {'id': create_result['event_id'], 'summary': f'🍪 Cookie Delivery - {order_number}', 'description': event_description, 'location': location, 'start': {'dateTime': start_datetime}, 'end': {'dateTime': end_datetime}, 'status': 'confirmed'}
                    if date in delivery_schedule:
                        delivery_schedule[date].append(event_data)
                    else:
                        delivery_schedule[date] = [event_data]
                    tool_context.state['delivery_schedule'] = delivery_schedule
                    return {'status': 'success', 'date': date, 'event_id': create_result['event_id'], 'event_link': create_result.get('event_link'), 'summary': f'🍪 Cookie Delivery - {order_number}', 'start_time': start_datetime, 'end_time': end_datetime, 'location': location, 'source': 'Google Calendar MCP', 'conflicts': availability_result.get('conflicts', [])}
                else:
    ...
    event = {'id': f'evt_{order_number}', 'summary': f'Cookie Delivery - {order_number}', 'description': f'Delivery for {customer_name} - Order {order_number}', 'location': location, 'start': {'dateTime': f"{date}T{slot['start']}-07:00"}, 'end': {'dateTime': f"{date}T{slot['end']}-07:00"}, 'status': 'confirmed'}
...
def send_confirmation_email(tool_context: ToolContext, recipient_email: str, subject: str, body: str) -> dict:
    ...
    if use_gmail_langchain and GMAIL_LANGCHAIN_AVAILABLE and gmail_manager:
        try:
            ...
            result = gmail_manager.send_email(to=recipient_email, subject=subject, body=body, body_type='html')
            if result['status'] == 'success':
                logging.info(f"LangChain Gmail: Successfully sent email {result['message_id']}")
                return {'status': 'success', 'recipient': recipient_email, 'subject': subject, 'message_id': result['message_id'], 'timestamp': result['timestamp'], 'method': 'langchain_gmail_toolkit'}
            else:
    ...
    return {'status': 'success', 'recipient': recipient_email, 'subject': subject, 'message_id': f"msg_{recipient_email}_{datetime.now().strftime('%Y%m%d_%H%M%S')}", 'timestamp': datetime.now().isoformat(), 'method': 'dummy_data'}
...
def check_delivery_availability(tool_context: ToolContext, date: str, time_preference: str='morning') -> dict:
    ...
    if use_calendar_mcp and CALENDAR_MCP_AVAILABLE and calendar_manager:
        try:
            ...
            availability_result = calendar_manager.check_availability(start_datetime=start_datetime, end_datetime=end_datetime, calendar_id=business_calendar_id)
            if availability_result['status'] == 'success':
                return {'status': 'success', 'available': availability_result['available'], 'conflicts': availability_result.get('conflicts', []), 'requested_date': date, 'requested_time': time_preference, 'time_slot': f"{slot['start']} - {slot['end']}", 'source': 'Google Calendar MCP'}
            else:
    ...
    for event in date_events:
        if 'delivery' in event.get('summary', '').lower():
            conflicts.append({'summary': event.get('summary', 'Existing Event'), 'start': event.get('start', {}).get('dateTime', ''), 'end': event.get('end', {}).get('dateTime', '')})
    is_available = len(conflicts) == 0
    return {'status': 'success', 'available': is_available, 'conflicts': conflicts, 'requested_date': date, 'requested_time': time_preference, 'time_slot': f"{slot['start']} - {slot['end']}", 'source': 'dummy_data'}
