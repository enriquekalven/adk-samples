...
def test_calendar_functions():
    ...
    try:
        ...
        if events_result['status'] == 'success':
            ...
            print(f" Failed to get events: {events_result.get('message', 'Unknown error')}")
        print()
        ...
        availability_result = calendar_manager.check_availability(start_iso, end_iso)
        print(f' Availability Result: {json.dumps(availability_result, indent=2)}')
        if availability_result['status'] == 'success':
            if availability_result['available']:
                print(' Time slot is available!')
            else:
                print(f" Time slot has conflicts: {len(availability_result['conflicts'])} conflicts")
        else:
            print(f"Failed to check availability: {availability_result.get('message', 'Unknown error')}")
        print()
        if availability_result.get('status') == 'success' and availability_result.get('available'):
            print(' Testing create_event (Organized Structure Test)...')
            create_result = calendar_manager.create_event(summary='MCP Organized Structure Test', description='This test event verifies the organized MCP server structure is working correctly. Created from mcp-servers/calendar/ subfolder.', location='456 Organized Drive, Clean Code City, CA', start_datetime=start_iso, end_datetime=end_iso)
            print(f'Create Event Result: {json.dumps(create_result, indent=2)}')
            if create_result['status'] == 'success':
                print('Test event created successfully from organized structure!')
                print(f"Event ID: {create_result.get('event_id')}")
                print(f"Event Link: {create_result.get('event_link', 'N/A')}")
            else:
                print(f"Failed to create event: {create_result.get('message', 'Unknown error')}")
        else:
            print('Skipping create_event test (time slot not available or availability check failed)')
        print()
