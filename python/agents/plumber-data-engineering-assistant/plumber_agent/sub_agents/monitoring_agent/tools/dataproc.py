...
def _handle_log_report(collected_logs: list[str], message_prefix: str='Fetched log entries') -> dict[str, str]:
    """
...
def get_dataproc_cluster_logs_with_name(project_id: str, dataproc_cluster_name: str='', _limit: int=10) -> dict[str, str]:
    """
    ...
    """
    try:
        collected_logs = get_dataproc_logs(project_id=project_id, resource_name='cloud_dataproc_cluster', query_str=dataproc_cluster_name, filter_key='cluster_name', _limit=_limit)
...
def get_dataproc_cluster_logs_with_id(project_id: str, dataproc_cluster_id: str, _limit: int=10) -> dict[str, str]:
    """
    ...
    """
    try:
        collected_logs = get_dataproc_logs(project_id=project_id, resource_name='cloud_dataproc_cluster', query_str=dataproc_cluster_id, filter_key='cluster_uuid', _limit=_limit)
        return _handle_log_report(collected_logs)
    except google_exceptions.GoogleAPIError as e:
        logger.error('A Google Cloud API error occurred: %s', e, exc_info=True)
        return {'status': 'error', 'report': f'A Google Cloud API error occurred: {e}'}
...
def get_dataproc_logs(project_id: str, resource_name: str='', query_str: str='', filter_key: str='', _limit: int=10) -> list[str]:
    """
    ...
    """
    full_filter = f'resource.type="{resource_name}" AND resource.labels.{filter_key}="{query_str.strip()}" AND timestamp >= "{(datetime.now() - timedelta(days=90)).isoformat()}Z"'
    collected_logs = []
    try:
        client = logging_v2.Client(project=project_id)
        project_path = f'projects/{project_id}'
        iterator = client.list_entries(resource_names=[project_path], order_by='timestamp desc', filter_=full_filter, max_results=_limit)
...
def get_dataproc_normal_job_logs_with_id(project_id: str, region: str, job_id: str, filter_key: str) -> subprocess.CompletedProcess | list[str]:
    """
    ...
    """
    command = ['gcloud', 'dataproc', filter_key, 'wait', job_id, '--project', project_id, '--region', region]
    ...
    try:
        ...
        logger.warning("gcloud command not found. Please ensure it is installed and in your system's PATH. %s", e, exc_info=True)
        return []
    except Exception as e:
...
def get_dataproc_job_logs_with_id(project_id: str, region: str, job_id: str) -> dict[str, str]:
    ...
    if not job_id or not project_id or (not region):
        return {'status': 'error', 'message': 'Missing required arguments (project_id, region, or job_id).'}
    result = None
    command_type = ''
    try:
        result = get_dataproc_normal_job_logs_with_id(project_id, region, job_id, 'batches')
        command_type = 'Batch'
        if isinstance(result, subprocess.CompletedProcess) and result.returncode != 0 and ('Not found' in result.stderr):
            result = get_dataproc_normal_job_logs_with_id(project_id, region, job_id, 'jobs')
            command_type = 'Job'
        if isinstance(result, list):
            return {'status': 'error', 'message': "Failed to execute 'gcloud' command. Check system PATH or installation."}
        if result.returncode == 0:
            report = f'Successfully waited for Dataproc {command_type} **{job_id}** to complete (gcloud exit code 0).\n\n--- Log Output (stdout) ---\n{result.stdout}\n\n--- Error/Warning Output (stderr) ---\n{result.stderr}'
            return {'status': 'success', 'report': report}
        report = f"Dataproc {command_type} **{job_id}** failed, was not found, or the 'gcloud wait' command timed out (gcloud exit code {result.returncode}).\n\n--- Console Output (stdout) ---\n{result.stdout}\n\n--- Error/Diagnostic Output (stderr) ---\n{result.stderr}"
        return {'status': 'error', 'message': f'gcloud command failed with exit code {result.returncode}.', 'report': report}
    except google_exceptions.GoogleAPIError as e:
        logger.error('A Google Cloud API error occurred: %s', e, exc_info=True)
        return {'status': 'error', 'message': f'Failed to get logs due to a Google Cloud API error: {e}'}
    except Exception as e:
        logger.error('An unexpected error occurred: %s', e, exc_info=True)
        return {'status': 'error', 'message': f'Failed to get logs due to an unexpected error: {e}'}