...
root_agent = Agent(name='dbt_agent', model=MODEL, description='Agent to convert source to target mapping image/csv file to corresponding dbt sql model', instruction=AGENT_INSTRUCTIONS, tools=[generate_dbt_model_sql, deploy_dbt_project, run_dbt_project], generate_content_config=types.GenerateContentConfig(safety_settings=[types.SafetySetting(category=types.HarmCategory.HARM_CATEGORY_DANGEROUS_CONTENT, threshold=types.HarmBlockThreshold.BLOCK_ONLY_HIGH)]))