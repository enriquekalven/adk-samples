...
from ..prompts import DATAFLOW_CODE_REVIEWER_EXPERT_PROMPT, STTM_PARSING_INSTRUCTIONS_BEAM_TRANSFORMATIONS
...
def generate_beam_transformations_from_sttm(gcs_path: str) -> str:
    ...
    try:
        ...
        response = model.generate_content([STTM_PARSING_INSTRUCTIONS_BEAM_TRANSFORMATIONS, file_content])
        ...
        logger.error('An error occurred while generating Beam transformations from STTM: %s', e, exc_info=True)
        return ''
...
async def create_pipeline_from_scratch(project_id: str, region: str, gcs_bucket_path: str, job_name: str, pipeline_code: str, pipeline_args: dict[str, str], pipeline_type: str) -> str:
    ...
    return await generate_and_run_beam_pipeline(project_id, region, gcs_bucket_path, job_name, reviewed_code, pipeline_args, pipeline_type)
...
async def generate_and_run_beam_pipeline(project_id: str, region: str, gcs_bucket_path: str, job_name: str, pipeline_code: str, pipeline_args: dict[str, str], pipeline_type: str) -> str:
    ...
    full_args = {'runner': 'DataflowRunner', 'project': project_id, 'region': region, 'job_name': sanitized_job_name, 'temp_location': os.path.join(gcs_bucket_path, 'temp'), 'staging_location': os.path.join(gcs_bucket_path, 'staging'), 'labels': json.dumps({'source': 'plumber'}), **pipeline_args}
    try:
        ...
        if pipeline_type == 'streaming':
            ...
            if process.stdout is not None:
                async for line_bytes in process.stdout:
                    ...
                    output += line
                    print(line, end='')
                    match = re.search('(\\d{4}-\\d{2}-\\d{2}_\\d{2}_\\d{2}_\\d{2}-\\d+)', line)
                    if match:
                        job_id_match = match
            ...
            if process.returncode != 0:
                raise subprocess.CalledProcessError(process.returncode or 1, command, output=output)
        job_id_match = re.search("jobId: '(\\d{4}-\\d{2}-\\d{2}_\\d{2}_\\d{2}_\\d{2}-\\d+)'", output)
        if not job_id_match:
            job_id_match = re.search('(?P<id>\\d{4}-\\d{2}-\\d{2}_\\d{2}_\\d{2}_\\d{2}-\\d+)', output)
        if not job_id_match:
        ...
        id_match = re.search("id: '([^']*)'", output)
        client_request_id_match = re.search("clientRequestId: '([^']*)'", output)
        create_time_match = re.search("createTime: '([^']*)'", output)
        ...
        final_report = {'status': status, 'report': '\n'.join(report_lines), 'job_id': job_id, 'gcs_script_path': gcs_path}
        ...
        logger.error('An error occurred: %s', e, exc_info=True)
        return json.dumps({'status': 'error', 'error_message': f'Unexpected error: {e!s}'})
    finally:
