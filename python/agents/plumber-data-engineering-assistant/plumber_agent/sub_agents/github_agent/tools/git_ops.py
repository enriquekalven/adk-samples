...
def initialize_git_repo(repo_path: str) -> dict[str, Any]:
    ...
    try:
        ...
        try:
            ...
            return {'status': 'success', 'message': f'Git repository already exists at {repo_path}', 'is_existing': True, 'current_branch': existing_repo.active_branch.name}
        except InvalidGitRepositoryError as e:
            logger.error('An error occurred: %s', e, exc_info=True)
            repo = Repo.init(repo_path)
            return {'status': 'success', 'message': f'Git repository initialized at {repo_path}', 'is_existing': False, 'current_branch': 'main' if repo.heads else 'No commits yet'}
    except Exception as e:
        logger.error('An error occurred: %s', e, exc_info=True)
        return {'status': 'error', 'message': f'Failed to initialize Git repository: {e!s}'}
...
def get_git_status(repo_path: str) -> dict[str, Any]:
    ...
    try:
        ...
        return {'status': 'success', 'current_branch': current_branch, 'has_commits': has_commits, 'is_dirty': repo.is_dirty(), 'modified_files': modified_files, 'staged_files': staged_files, 'untracked_files': list(untracked_files), 'total_changes': len(modified_files) + len(staged_files) + len(untracked_files)}
    except InvalidGitRepositoryError as e:
        logger.error('An error occurred: %s', e, exc_info=True)
        return {'status': 'error', 'message': f'Not a Git repository: {repo_path}'}
    except Exception as e:
        logger.error('An error occurred: %s', e, exc_info=True)
        return {'status': 'error', 'message': f'Failed to get Git status: {e!s}'}

def add_files_to_git(repo_path: str, files: list[str] | None=None, add_all: bool=False) -> dict[str, Any]:
    """
    ...
    try:
        ...
        if add_all:
            ...
            return {'status': 'error', 'message': 'No files specified and add_all is False'}
        return {'status': 'success', 'message': 'Files added to staging area successfully', 'added_files': added_files}
    except InvalidGitRepositoryError as e:
        logger.error('An error occurred: %s', e, exc_info=True)
        return {'status': 'error', 'message': f'Not a Git repository: {repo_path}'}
    except Exception as e:
...
def commit_changes(repo_path: str, commit_message: str, author_name: str='', author_email: str='') -> dict[str, Any]:
    """
    ...
    try:
        ...
        return {'status': 'success', 'message': 'Changes committed successfully', 'commit_hash': commit.hexsha[:8], 'commit_message': commit_message, 'files_changed': len(commit.stats.files)}
    except InvalidGitRepositoryError as e:
        logger.error('An error occurred: %s', e, exc_info=True)
        return {'status': 'error', 'message': f'Not a Git repository: {repo_path}'}
    except Exception as e:
        logger.error('An error occurred: %s', e, exc_info=True)
        return {'status': 'error', 'message': f'Failed to commit changes: {e!s}'}
...
def list_git_branches(repo_path: str) -> dict[str, Any]:
    ...
    try:
        ...
        for branch in repo.heads:
            branch_info = {'name': branch.name, 'is_current': branch == repo.active_branch}
            if branch_info['is_current']:
                current_branch = branch.name
            branches.append(branch_info)
        return {'status': 'success', 'current_branch': current_branch, 'branches': branches, 'total_branches': len(branches)}
    except InvalidGitRepositoryError as e:
        logger.error('An error occurred: %s', e, exc_info=True)
        return {'status': 'error', 'message': f'Not a Git repository: {repo_path}'}
    except Exception as e:
...
def switch_git_branch(repo_path: str, branch_name: str, create_if_not_exists: bool=False) -> dict[str, Any]:
    """
    ...
    try:
        repo = Repo(repo_path)
        if repo.is_dirty():
            msg = 'Cannot switch branches with uncommitted changes. Please commit or stash changes first.'
            return {'status': 'error', 'message': msg}
        try:
            repo.git.checkout(branch_name)
            return {'status': 'success', 'message': f"Switched to existing branch '{branch_name}'", 'current_branch': branch_name, 'created_new': False}
        except git.exc.GitCommandError as e:
            logger.error('An error occurred: %s', e, exc_info=True)
            if create_if_not_exists:
                repo.git.checkout('-b', branch_name)
                return {'status': 'success', 'message': f"Created and switched to new branch '{branch_name}'", 'current_branch': branch_name, 'created_new': True}
            msg = f"Branch '{branch_name}' does not exist. Set create_if_not_exists=True to create it."
            return {'status': 'error', 'message': msg}
    except InvalidGitRepositoryError as e:
        logger.error('An error occurred: %s', e, exc_info=True)
        return {'status': 'error', 'message': f'Not a Git repository: {repo_path}'}
