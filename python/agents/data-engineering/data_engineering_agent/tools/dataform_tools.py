...
def get_workspace_path() -> str:
    ...
    return DATAFORM_CLIENT.workspace_path(config.project_id, config.location, config.repository_name, config.workspace_name)
...
def write_file_to_dataform(file_content: str, file_path: str) -> str:
    ...
    try:
        request = dataform_v1.WriteFileRequest(workspace=workspace_path, path=file_path, contents=file_content.encode('utf-8'))
...
def delete_file_from_dataform(file_path: str) -> str:
    ...
    """
    workspace_path = get_workspace_path()
    request = dataform_v1.RemoveFileRequest(workspace=workspace_path, path=file_path)
    try:
...
def compile_dataform(compile_only: bool=False) -> dict[str, Any]:
    ...
    """
    try:
        repository_path = DATAFORM_CLIENT.repository_path(config.project_id, config.location, config.repository_name)
        workspace_path = get_workspace_path()
        print('Compiling...')
        ...
        request = dataform_v1.CreateCompilationResultRequest(parent=repository_path, compilation_result=compilation_result)
        ...
        if compilation_results.compilation_errors:
            ...
            return {'status': 'error', 'error_message': str(compilation_results.compilation_errors)}
        request = dataform_v1.QueryCompilationResultActionsRequest(name=compilation_results.name)
        actions = DATAFORM_CLIENT.query_compilation_result_actions(request=request).compilation_result_actions
        if compile_only:
            return {'status': 'success', 'message': 'Compilation successful (compile-only mode)', 'pipeline_dag': str(actions)}
        workflow_invocation = dataform_v1.WorkflowInvocation()
        workflow_invocation.compilation_result = compilation_results.name
        request = dataform_v1.CreateWorkflowInvocationRequest(parent=repository_path, workflow_invocation=workflow_invocation)
        workflow_invocation = DATAFORM_CLIENT.create_workflow_invocation(request=request)
        return {'status': 'success', 'message': 'Compilation and execution successful', 'pipeline_dag': str(actions), 'workflow_invocation_id': workflow_invocation.name}
    except GoogleAPIError as e:
...
def read_file_from_dataform(file_path: str) -> str:
    ...
    try:
        request = dataform_v1.ReadFileRequest(workspace=workspace_path, path=file_path)
...
def get_dataform_execution_logs(workflow_invocation_id: str) -> dict[str, Any]:
    ...
    """
    try:
        request = dataform_v1.QueryWorkflowInvocationActionsRequest(name=workflow_invocation_id)
        actions_response = DATAFORM_CLIENT.query_workflow_invocation_actions(request=request)
        actions_details = []
        for action in actions_response.workflow_invocation_actions:
            action_detail = {'name': action.target.name, 'status': dataform_v1.WorkflowInvocationAction.State(action.state).name}
            if action.state == dataform_v1.WorkflowInvocationAction.State.FAILED:
                action_detail['error_message'] = action.failure_reason
            if action.canonical_target.name:
                action_detail['canonical_target_name'] = action.canonical_target.name
            if action.bigquery_action:
        ...
        if overall_status == 'error':
            return {'status': 'error', 'error_message': f'One or more actions failed in workflow invocation {workflow_invocation_id}. See actions for details.', 'actions': actions_details}
        return {'status': 'success', 'actions': actions_details}
    except GoogleAPIError as e:
        print(f"Error getting execution logs for '{workflow_invocation_id}': {e}")
        return {'status': 'error', 'error_message': f"Error getting execution logs for '{workflow_invocation_id}': {e}"}

def execute_dataform_workflow(workflow_name: str, params: dict[str, Any] | None=None) -> dict[str, Any]:
    ...
    try:
        ...
        if params:
            workflow_invocation.invocation_config = dataform_v1.InvocationConfig(parameters=params)
        request = dataform_v1.CreateWorkflowInvocationRequest(parent=repository_path, workflow_invocation=workflow_invocation)
        workflow_invocation = DATAFORM_CLIENT.create_workflow_invocation(request=request)
        return {'status': 'success', 'message': 'Workflow execution started', 'workflow_invocation_id': workflow_invocation.name, 'workflow_name': workflow_name, 'parameters': params}
    except GoogleAPIError as e:
...
def get_dataform_repo_link() -> dict[str, str]:
    ...
    try:
        ...
        return {'status': 'success', 'repository_url': repo_url, 'repository_name': config.repository_name, 'project_id': config.project_id, 'location': config.location, 'workspace_name': config.workspace_name}
